...
struct test_chain
{
   ...
   void on_accepted_block(const block_state_ptr& block_state)
   {
      ...
      state_history::trace_converter::pack(strm, control->db(), false,
                                           trace_cache.prepare_traces(block_state),
...
struct callbacks
{
                         ...
                         uint32_t cb_alloc)
   {
      ...
      auto result =
          chain.control->push_transaction(fut.get(), fc::time_point::maximum(), 2000, true);
   ...
   void rodeos_enable_queries(uint32_t rodeos,
   ...
   void rodeos_push_transaction(uint32_t rodeos,
   ...
   uint32_t kv_it_create(uint64_t contract, span<const char> prefix)
   {
      ...
      else
      {
         ...
         EOS_ASSERT(selected().kv_iterators.size() <= 0xFFFFFFFFu, kv_bad_iter,
                    "Too many iterators");
   ...
   int32_t kv_it_next(uint32_t itr,
                      wasm_ptr<uint32_t> found_key_size,
                      wasm_ptr<uint32_t> found_value_size)
   {
      kv_check_iterator(itr);
      return static_cast<int32_t>(
          selected().kv_iterators[itr]->kv_it_next(found_key_size, found_value_size));
   }
                      ...
                      wasm_ptr<uint32_t> found_value_size)
   {
      kv_check_iterator(itr);
      return static_cast<int32_t>(
          selected().kv_iterators[itr]->kv_it_prev(found_key_size, found_value_size));
   }
   ...
   {
      kv_check_iterator(itr);
      return static_cast<int32_t>(selected().kv_iterators[itr]->kv_it_lower_bound(
   ...
   int32_t kv_it_value(uint32_t itr,
                       uint32_t offset,
                       span<char> dest,
                       wasm_ptr<uint32_t> actual_size)
   {
      kv_check_iterator(itr);
      return static_cast<int32_t>(selected().kv_iterators[itr]->kv_it_value(
          offset, dest.data(), dest.size(), *actual_size));
   }

   kv_context& kv_get_db() { return selected().kv_get_backing_store(); }
