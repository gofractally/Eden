...
namespace b1
{
   ...
   void rocksdb_plugin::set_program_options(options_description& cli, options_description& cfg)
   {
      ...
      op("rdb-threads", bpo::value<uint32_t>(),
         "Increase number of background RocksDB threads. Only used with cloner_plugin. Recommend 8 "
         "for full history "
         "on large chains.");
      op("rdb-max-files", bpo::value<uint32_t>(),
         "RocksDB limit max number of open files (default unlimited). This should be smaller than "
         "'ulimit -n #'. "
   ...
   std::shared_ptr<chain_kv::database> rocksdb_plugin::get_db()
   {
      ...
      if (!my->database)
      {
         ...
         my->database = std::make_shared<chain_kv::database>(my->db_path.c_str(), true, my->threads,
                                                             my->max_open_files);
      }
      return my->database;
   }
