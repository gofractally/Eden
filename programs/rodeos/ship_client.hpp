...
namespace b1::ship_client
{
   ...
   struct connection_callbacks
   {
      ...
      virtual bool received(ship::get_status_result_v0& status, eosio::input_stream bin)
      {
         return true;
      }
      virtual bool received(ship::get_blocks_result_v0& result, eosio::input_stream bin)
      {
         return true;
      }
#if 0
   ...
   EOSIO_REFLECT(abi_def_skip_table,
                 version,
                 types,
                 structs,
                 actions,
   ...
   struct connection : std::enable_shared_from_this<connection>
   {
      ...
      void start_read()
      {
         ...
         stream.async_read(*in_buffer,
                           [self = shared_from_this(), this, in_buffer](error_code ec, size_t) {
      ...
      bool receive_result(const std::shared_ptr<flat_buffer>& p)
      {
         ...
         from_bin(result, bin);
         return callbacks &&
                std::visit([&](auto& r) { return callbacks->received(r, orig); }, result);
      }

      void request_blocks(uint32_t start_block_num,
                          const std::vector<ship::block_position>& positions,
                          int flags)
      {
      ...
      void request_blocks(const ship::get_status_result_v0& status,
      ...
      void send(const ship::request& req)
      {
         ...
         stream.async_write(boost::asio::buffer(*bin),
