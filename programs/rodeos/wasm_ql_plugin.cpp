...
void wasm_ql_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   ...
   op("wql-listen", bpo::value<std::string>()->default_value("127.0.0.1:8880"),
      "Endpoint to listen on");
   op("wql-retries", bpo::value<uint32_t>()->default_value(0xffff'ffff),
      "Number of times to retry binding to --wql-listen. Each retry is approx 1 second apart. Set "
      "to 0 to prevent "
      "retries.");
   op("wql-allow-origin", bpo::value<std::string>(),
      "Access-Control-Allow-Origin header. Use \"*\" to allow any.");
   op("wql-contract-dir", bpo::value<std::string>(),
      "Directory to fetch contracts from. These override contracts on the chain. (default: "
      "disabled)");
   op("wql-static-dir", bpo::value<std::string>(),
      "Directory to serve static files from (default: disabled)");
   op("wql-query-mem", bpo::value<uint32_t>()->default_value(33),
      "Maximum size of wasm memory (MiB)");
   op("wql-console-size", bpo::value<uint32_t>()->default_value(0), "Maximum size of console data");
   op("wql-wasm-cache-size", bpo::value<uint32_t>()->default_value(100),
      "Maximum number of compiled wasms to cache");
   op("wql-max-request-size", bpo::value<uint32_t>()->default_value(10000),
      "HTTP maximum request body size (bytes)");
   op("wql-idle-timeout", bpo::value<uint64_t>()->default_value(30000),
   ...
   op("wql-checkpoint-dir", bpo::value<boost::filesystem::path>(),
      "Directory to place checkpoints. Caution: this allows anyone to create a checkpoint using "
      "RPC (default: "
      "disabled)");
}
...
void wasm_ql_plugin::plugin_initialize(const variables_map& options)
{
   try
   {
      ...
      auto shared_state =
          std::make_shared<wasm_ql::shared_state>(app().find_plugin<rocksdb_plugin>()->get_db());
...
void wasm_ql_plugin::plugin_shutdown()
{
   my->shutdown();
}
