...
namespace b1::rodeos::wasm_ql
{
   ...
   void handle_request(const wasm_ql::http_config& http_config,
                       const wasm_ql::shared_state& shared_state,
                       thread_state_cache& state_cache,
                       ...
                       Send&& send)
   {
      ...
      try
      {
         ...
         else if (req.target() == "/v1/chain/get_block")
         {  // todo: replace with /v1/chain/get_block_header. upgrade cleos.
            if (req.method() != http::verb::post)
               return send(error(http::status::bad_request,
         ...
         else if (req.target() == "/v1/chain/get_abi")
         ...
         else if (req.target() == "/v1/chain/get_required_keys")
         {  // todo: replace with a binary endpoint?
            if (req.method() != http::verb::post)
               return send(error(http::status::bad_request,
                                 "Unsupported HTTP-method for " + req.target().to_string() + "\n"));
            auto thread_state = state_cache.get_state();
            send(ok(query_get_required_keys(*thread_state,
                                            std::string_view{req.body().data(), req.body().size()}),
                    "application/json"));
            return;
         }
         else if (req.target() == "/v1/chain/send_transaction")
         {
            ...
            if (req.method() != http::verb::post)
               return send(error(http::status::bad_request,
                                 "Unsupported HTTP-method for " + req.target().to_string() + "\n"));
            auto thread_state = state_cache.get_state();
            send(ok(query_send_transaction(
                        *thread_state, temp_contract_kv_prefix,
                        std::string_view{req.body().data(), req.body().size()},
         ...
         else if (req.target() == "/v1/rodeos/create_checkpoint")
         {
            ...
            auto thread_state = state_cache.get_state();
            send(ok(query_create_checkpoint(*thread_state, *http_config.checkpoint_dir),
                    "application/json"));
            return;
         }
         ...
         {
            // todo: redirect if /v1/?
            return send(error(http::status::not_found,
         ...
         else
         {
            ...
            if (ec)
               return send(error(http::status::internal_server_error,
      ...
      catch (...)
      {
         ...
         return send(
             error(http::status::internal_server_error, "query failed: unknown exception\n"));
      }
   }  // handle_request
         ...
         void operator()(http::message<isRequest, Body, Fields>&& msg)
         {
            ...
            struct work_impl : work
            {
               ...
               void operator()()
               {
                  http::async_write(
                      self.stream, msg,
                      beast::bind_front_handler(&http_session::on_write, self.shared_from_this(),
                                                msg.need_eof()));
               }
            };
     ...
     private:
      ...
      void on_accept(beast::error_code ec, tcp::socket socket)
      {
         ...
         else
         {
            ...
            std::make_shared<http_session>(http_config, shared_state, state_cache,
                                           std::move(socket))
                ->run();
         }
