...
namespace b1
{
   ...
   struct cloner_session : ship_client::connection_callbacks,
                           std::enable_shared_from_this<cloner_session>
   {
      ...
      cloner_session(cloner_plugin_impl* my) : my(my), config(my->config)
      {
         ...
         if (!config->filter_wasm.empty())
            filter = std::make_unique<rodeos_filter>(
                config->filter_name, config->filter_wasm, config->profile
#ifdef EOSIO_EOS_VM_OC_RUNTIME_ENABLED
                ,
      ...
      std::vector<block_position> get_positions()
      {
         ...
         {
            rodeos::db_view_state view_state{rodeos::state_account, *db,
                                             *rodeos_snapshot->write_session,
            ...
            for (uint32_t i = rodeos_snapshot->irreversible; i <= rodeos_snapshot->head; ++i)
            {
               auto info = rodeos::get_state_row<rodeos::block_info>(
                   view_state.kv_state.view,
                   std::make_tuple(eosio::name{"block.info"}, eosio::name{"primary"}, i));
               if (!info)
                  throw std::runtime_error("database is missing block.info for block " +
                                           std::to_string(i));
      ...
      bool process_received(Get_Blocks_Result& result, eosio::input_stream bin)
      {
         ...
         if (rodeos_snapshot->head && result.this_block->block_num > rodeos_snapshot->head + 1)
            throw std::runtime_error("state-history plugin is missing block " +
                                     std::to_string(rodeos_snapshot->head + 1));
         ...
         if (write_now || !reported_block)
            ilog("block ${b} ${i}",
                 ("b", result.this_block->block_num)(
                     "i", result.this_block->block_num <= result.last_irreversible.block_num
                              ? "irreversible"
                              : ""));
         reported_block = true;
   ...
   void cloner_plugin::set_program_options(options_description& cli, options_description& cfg)
   {
      ...
      op("eos-vm-oc-cache-size-mb",
         bpo::value<uint64_t>()->default_value(eosio::chain::eosvmoc::config().cache_size /
                                               (1024u * 1024u)),
         "Maximum size (in MiB) of the EOS VM OC code cache");
      op("eos-vm-oc-compile-threads",
         bpo::value<uint64_t>()->default_value(1u)->notifier([](const auto t) {
            if (t == 0)
            {
   ...
   void cloner_plugin::plugin_initialize(const variables_map& options)
   {
      try
      {
         ...
         my->config->port = port;
         my->config->skip_to =
             options.count("clone-skip-to") ? options["clone-skip-to"].as<uint32_t>() : 0;
         my->config->stop_before =
             options.count("clone-stop") ? options["clone-stop"].as<uint32_t>() : 0;
         my->config->exit_on_filter_wasm_error =
             options["clone-exit-on-filter-wasm-error"].as<bool>();
         ...
         if (options.count("eos-vm-oc-cache-size-mb"))
            my->config->eosvmoc_config.cache_size =
                options.at("eos-vm-oc-cache-size-mb").as<uint64_t>() * 1024u * 1024u;
         if (options.count("eos-vm-oc-compile-threads"))
            my->config->eosvmoc_config.threads =
                options.at("eos-vm-oc-compile-threads").as<uint64_t>();
