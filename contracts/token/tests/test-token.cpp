...
struct token_tester
{
   ...
   token_tester(const call_stack* stack)
   {
      chain.create_account(H, "alice"_n);
      chain.create_account(H, "bob"_n);
      chain.create_account(H, "carol"_n);
      chain.create_code_account(H, "eosio.token"_n);
      chain.set_code(H, "eosio.token"_n, "token.wasm");
   }
...
TEST_CASE("Create a token", "[create]")
{
   ...
   t.eosio_token.act<actions::create>(H, "alice"_n, s2a("1000.000 TKN"));
...
{
   token_tester t(H);
   expect(H, t.eosio_token.trace<actions::create>("alice"_n, s2a("-1000.000 TKN")),
...
TEST_CASE("Create token with a symbol that already exists", "[symbol_already_exists]")
{
   ...
   t.eosio_token.act<actions::create>(H, "alice"_n, s2a("1000 TKN"));
   REQUIRE(t.get_stats(symbol_code{"TKN"}) ==  //
           (currency_stats{
               .supply = s2a("0 TKN"),
   ...
   expect(H, t.eosio_token.trace<actions::create>("alice"_n, s2a("100 TKN")),
...
TEST_CASE("Create a token whose max supply is to large", "[create_max_supply]")
{
   ...
   t.eosio_token.act<actions::create>(H, "alice"_n, s2a("4611686018427387903 TKN"));
   REQUIRE(t.get_stats(symbol_code{"TKN"}) ==  //
           (currency_stats{
               .supply = s2a("0 TKN"),
   ...
   ++too_big.amount;
   expect(H, t.eosio_token.trace<actions::create>("alice"_n, too_big), "invalid supply");
}
...
TEST_CASE("Create a token whose precision is too high", "[precision_too_high]")
{
   ...
   t.eosio_token.act<actions::create>(H, "alice"_n, asset{1, symbol{"TKN", 18}});
   REQUIRE(t.get_stats(symbol_code{"TKN"}) ==  //
           (currency_stats{
   ...
   t.eosio_token.act<actions::create>(H, "alice"_n, asset{1, symbol{"NKT", 50}});
}
...
TEST_CASE("Test issuing a token", "[issue_tests]")
{
   ...
   t.eosio_token.act<actions::create>(H, "alice"_n, s2a("1000.000 TKN"));
   t.alice.act<actions::issue>(H, "alice"_n, s2a("500.000 TKN"), "hola");
   REQUIRE(t.get_stats(symbol_code{"TKN"}) ==  //
           (currency_stats{
   ...
   REQUIRE(token::contract::get_balance("eosio.token"_n, "alice"_n, symbol_code{"TKN"}) ==
           s2a("500.000 TKN"));
   expect(H, t.alice.trace<actions::issue>("alice"_n, s2a("500.001 TKN"), "hola"),
          "quantity exceeds available supply");
   expect(H, t.alice.trace<actions::issue>("alice"_n, s2a("-1.000 TKN"), "hola"),
          "must issue positive quantity");
   t.alice.act<actions::issue>(H, "alice"_n, s2a("1.000 TKN"), "hola");
}
...
TEST_CASE("Retire tokens", "[retire_tests]")
{
   ...
   REQUIRE(token::contract::get_balance("eosio.token"_n, "alice"_n, symbol_code{"TKN"}) ==
           s2a("500.000 TKN"));
   t.alice.act<actions::retire>(H, s2a("200.000 TKN"), "hola");
   ...
   expect(H, t.alice.trace<actions::retire>(s2a("500.000 TKN"), "hola"), "overdrawn balance");

   t.alice.act<actions::transfer>(H, "alice"_n, "bob"_n, s2a("200.000 TKN"), "hola");
   // should fail to retire since tokens are not on the issuer's balance
   expect(H, t.alice.trace<actions::retire>(s2a("300.000 TKN"), "hola"), "overdrawn balance");
   // transfer tokens back
   t.bob.act<actions::transfer>(H, "bob"_n, "alice"_n, s2a("200.000 TKN"), "hola");

   t.alice.act<actions::retire>(H, s2a("300.000 TKN"), "hola");
   REQUIRE(t.get_stats(symbol_code{"TKN"}) ==  //
           (currency_stats{
               .supply = s2a("0.000 TKN"),
               .max_supply = s2a("1000.000 TKN"),
   ...
   expect(H, t.alice.trace<actions::retire>(s2a("1.000 TKN"), "hola"), "overdrawn balance");
}
...
TEST_CASE("Transfer tokens", "[transfer_tests]")
{
   ...
   t.eosio_token.act<actions::create>(H, "alice"_n, s2a("1000 CERO"));
   t.alice.act<actions::issue>(H, "alice"_n, s2a("1000 CERO"), "hola");
   ...
   REQUIRE(token::contract::get_balance("eosio.token"_n, "alice"_n, symbol_code{"CERO"}) ==
           s2a("1000 CERO"));
   t.alice.act<actions::transfer>(H, "alice"_n, "bob"_n, s2a("300 CERO"), "hola");
   REQUIRE(token::contract::get_balance("eosio.token"_n, "alice"_n, symbol_code{"CERO"}) ==
           s2a("700 CERO"));
   REQUIRE(token::contract::get_balance("eosio.token"_n, "bob"_n, symbol_code{"CERO"}) ==
           s2a("300 CERO"));

   expect(H, t.alice.trace<actions::transfer>("alice"_n, "bob"_n, s2a("701 CERO"), "hola"),
          "overdrawn balance");
   expect(H, t.alice.trace<actions::transfer>("alice"_n, "bob"_n, s2a("-1000 CERO"), "hola"),
...
TEST_CASE("Open token balance", "[open_tests]")
{
   ...
   expect(H, t.alice.trace<actions::issue>("bob"_n, s2a("1000 CERO"), ""),
          "tokens can only be issued to issuer account");
   t.alice.act<actions::issue>(H, "alice"_n, s2a("1000 CERO"), "issue");

   REQUIRE(token::contract::get_balance("eosio.token"_n, "alice"_n, symbol_code{"CERO"}) ==
           s2a("1000 CERO"));
   REQUIRE(t.get_account_optional("bob"_n, symbol_code("CERO")) == std::nullopt);

   expect(H, t.alice.trace<actions::open>("nonexistent"_n, symbol{"CERO", 0}, "alice"_n),
          "owner account does not exist");
   t.alice.act<actions::open>(H, "bob"_n, symbol{"CERO", 0}, "alice"_n);

   REQUIRE(token::contract::get_balance("eosio.token"_n, "bob"_n, symbol_code{"CERO"}) ==
           s2a("0 CERO"));
   t.alice.act<actions::transfer>(H, "alice"_n, "bob"_n, s2a("200 CERO"), "hola");
   REQUIRE(token::contract::get_balance("eosio.token"_n, "bob"_n, symbol_code{"CERO"}) ==
           s2a("200 CERO"));

   expect(H, t.alice.trace<actions::open>("carol"_n, symbol{"INVALID", 0}, "alice"_n),
          "symbol does not exist");
   expect(H, t.alice.trace<actions::open>("carol"_n, symbol{"CERO", 1}, "alice"_n),
...
TEST_CASE("Close token balance", "[close_tests]")
{
   ...
   t.alice.act<actions::transfer>(H, "alice"_n, "bob"_n, s2a("1000 CERO"), "hola");
   REQUIRE(token::contract::get_balance("eosio.token"_n, "alice"_n, symbol_code{"CERO"}) ==
           s2a("0 CERO"));

   t.alice.act<actions::close>(H, "alice"_n, symbol{"CERO", 0});
   REQUIRE(t.get_account_optional("alice"_n, symbol_code("CERO")) == std::nullopt);
}
