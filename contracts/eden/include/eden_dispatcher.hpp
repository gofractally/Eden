...
namespace eden
{
   ...
   void execute_auth_action(eosio::name contract,
                            R (T::*func)(const eosio::excluded_arg<session_info>& current_session,
                                         Args...),
...
#define EDEN_DISPATCH_AUTH_ACTION_INTERNAL_1(r, type, member)                         \
   case EDEN_EXTRACT_AUTH_ACTION_INDEX(member):                                       \
      ::eden::execute_auth_action(contract, &type::EOSIO_EXTRACT_ACTION_NAME(member), \
                                  current_session, ds);                               \
      return true;
...
#define EDEN_GET_AUTH_ACTION_INTERNAL_1(r, type, member)    \
   f(EDEN_EXTRACT_AUTH_ACTION_INDEX(member),                \
     BOOST_PP_STRINGIZE(EOSIO_EXTRACT_ACTION_NAME(member)), \
     &type::EOSIO_EXTRACT_ACTION_NAME(member));
#define EDEN_GET_AUTH_ACTION_INTERNAL(r, type, member)                                        \
   BOOST_PP_IIF(EDEN_MATCH_AUTH_ACTION(member), EDEN_GET_AUTH_ACTION_INTERNAL_1, EOSIO_EMPTY) \
   (r, type, member)
#define EDEN_GET_AUTH_ACTION(type, MEMBERS) \
   BOOST_PP_SEQ_FOR_EACH(EDEN_GET_AUTH_ACTION_INTERNAL, type, MEMBERS)

#define EDEN_NAME_FOR_AUTH_ACTION_INTERNAL_1(r, type, member) \
   case EDEN_EXTRACT_AUTH_ACTION_INDEX(member):               \
      return BOOST_PP_CAT(BOOST_PP_STRINGIZE(EOSIO_EXTRACT_ACTION_NAME(member)), _n);
#define EDEN_NAME_FOR_AUTH_ACTION_INTERNAL(r, type, member)                                        \
   BOOST_PP_IIF(EDEN_MATCH_AUTH_ACTION(member), EDEN_NAME_FOR_AUTH_ACTION_INTERNAL_1, EOSIO_EMPTY) \
   (r, type, member)
