    ...
    <SideNavLayout title="Election video upload service">
      <div className="divide-y">
        ...
        <Container className="space-y-6 pb-5">
          {isUploadExpired ? (
            <div>Election is complete and upload videos time is expired.</div>
          ) : (
            <>
              <Text>
        ...
        return (
          <div key={`video-round-${vote.roundIndex}`}>
            <Expander
              header={
                <Header
                  isOngoing={vote.votingStarted && !vote.votingFinished}
            ...
            >
              <Container>
                <VideoSubmissionFormAndPreview
                  uid={vote.roundIndex}
                  video={vote.video}
                  onSubmit={submitElectionRoundVideo(vote.roundIndex)}
