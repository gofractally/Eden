...
namespace eosio
{
   ...
   {
      return (magic & 0xffff'ffff'0000'0000) == N(ship).to_uint64_t();
   }
   ...
   static const int state_history_log_header_serial_size =
       sizeof(state_history_log_header::magic) + sizeof(state_history_log_header::block_id) +
   ...
   class state_history_log
   {
     ...
     public:
      state_history_log(const char* const name,
                        std::string log_filename,
                        std::string index_filename)
          : name(name),
      ...
      template <typename F>
      void write_entry(const state_history_log_header& header,
                       ...
                       F write_payload)
      {
         ...
         if (_begin_block != _end_block && block_num > _begin_block)
         {
            if (block_num == _end_block)
            {
               EOS_ASSERT(prev_id == last_block_id, chain::plugin_exception,
                          "missed a fork change in ${name}.log", ("name", name));
            }
            else
            {
               state_history_log_header prev;
               get_entry(block_num - 1, prev);
               EOS_ASSERT(prev_id == prev.block_id, chain::plugin_exception,
                          "missed a fork change in ${name}.log", ("name", name));
            }
         }
         ...
         EOS_ASSERT(end == pos + state_history_log_header_serial_size + header.payload_size,
                    chain::plugin_exception, "wrote payload with incorrect size to ${name}.log",
                    ("name", name));
     ...
     private:
      bool get_last_block(uint64_t size)
      {
         ...
         if (!is_ship(header.magic) || !is_ship_supported_version(header.magic) ||
             suffix + state_history_log_header_serial_size + header.payload_size + sizeof(suffix) !=
                 size)
         {
      ...
      void recover_blocks(uint64_t size)
      {
         ...
         while (true)
         {
            ...
            if (!is_ship(header.magic) || !is_ship_supported_version(header.magic) ||
                ...
                pos + state_history_log_header_serial_size + header.payload_size + sizeof(suffix) >
                    size)
            {
               EOS_ASSERT(!is_ship(header.magic) || is_ship_supported_version(header.magic),
                          chain::plugin_exception, "${name}.log has an unsupported version",
                          ("name", name));
               break;
            }
            ...
            if (!(++num_found % 10000))
            {
               printf("%10u blocks found, log pos=%12llu\r", (unsigned)num_found,
         ...
         log.flush();
         EOS_ASSERT(get_last_block(pos), chain::plugin_exception, "recover ${name}.log failed",
                    ("name", name));
      }
      ...
      void open_log()
      {
         ...
         log.open("a+b");  // std::ios_base::binary | std::ios_base::in | std::ios_base::out |
                           // std::ios_base::app
         log.seek_end(0);
         ...
         if (size >= state_history_log_header_serial_size)
         {
            ...
            read_header(header, false);
            EOS_ASSERT(
                is_ship(header.magic) && is_ship_supported_version(header.magic) &&
                    state_history_log_header_serial_size + header.payload_size + sizeof(uint64_t) <=
                        size,
            ...
            if (!get_last_block(size))
               recover_blocks(size);
            ilog("${name}.log has blocks ${b}-${e}",
                 ("name", name)("b", _begin_block)("e", _end_block - 1));
         }
         else
         {
      ...
      void open_index()
      {
         ...
         index.open("a+b");  // std::ios_base::binary | std::ios_base::in | std::ios_base::out |
                             // std::ios_base::app
         index.seek_end(0);
         ...
         index.close();
         index.open("w+b");  // std::ios_base::binary | std::ios_base::in | std::ios_base::out |
                             // std::ios_base::trunc
         ...
         while (pos < size)
         {
            ...
            //      ("b", header.block_num)("pos", pos)("end", suffix_pos +
            //      sizeof(suffix))("suffix", suffix)("fs", size));
            EOS_ASSERT(suffix == pos, chain::plugin_exception, "corrupt ${name}.log (8)",
                       ("name", name));
      ...
      void truncate(uint32_t block_num)
      {
         ...
         else
         {
            ...
            index.seek(0);
            boost::filesystem::resize_file(log_filename, pos);
            boost::filesystem::resize_file(index_filename,
                                           (block_num - _begin_block) * sizeof(uint64_t));
            _end_block = block_num;
         }
         log.flush();
         index.flush();
         ilog("fork or replay: removed ${n} blocks from ${name}.log",
              ("n", num_removed)("name", name));
      }
   };  // state_history_log
