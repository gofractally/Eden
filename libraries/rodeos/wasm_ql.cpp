...
namespace b1::rodeos::wasm_ql
{
   ...
   using backend_container = multi_index_container<
       ...
       indexed_by<
           ...
           ordered_non_unique<tag<by_name>,
                              member<backend_entry, eosio::name, &backend_entry::name>>,
           ordered_non_unique<tag<by_hash>,
                              member<backend_entry, eosio::checksum256, &backend_entry::hash>>>>;
   ...
   std::optional<std::vector<uint8_t>> read_code(wasm_ql::thread_state& thread_state,
                                                 eosio::name account)
   {
   ...
   std::optional<eosio::checksum256> get_contract_hash(db_view_state& db_view_state,
                                                       eosio::name account)
   {
   ...
   std::optional<std::vector<uint8_t>> read_contract(db_view_state& db_view_state,
                                                     const eosio::checksum256& hash,
                                                     eosio::name account)
   {
      std::optional<std::vector<uint8_t>> result;
      auto code_row = get_state_row<ship_protocol::code>(
          db_view_state.kv_state.view, std::make_tuple(eosio::name{"code"}, eosio::name{"primary"},
                                                       uint8_t(0), uint8_t(0), hash));
      if (!code_row)
         return result;
                   ...
                   std::vector<std::vector<char>>& memory)
   {
      ...
      auto se =
          fc::make_scoped_exit([&] { thread_state.shared->backend_cache->add(std::move(*entry)); });
      ...
      // todo: move these out of thread_state since future enhancements could cause state to
      // accidentally leak between queries
      ...
      try
      {
         ...
         entry->backend->timed_run(wd, [&] {
            ...
            (*entry->backend)(cb, "env", "apply", action.account.value, action.account.value,
                              action.name.value);
         });
      }
      catch (...)
            ...
            std::visit(
                [&](auto& obj) {
                   ...
                   result += ",\"last_irreversible_block_num\":\"" +
                             std::to_string(obj.irreversible) + "\"";
                                            ...
                                            std::string_view body)
   {
      ...
      if (params.block_num_or_id.size() == 64)
      {
         ...
         {
            throw std::runtime_error("An error occurred deserializing block_num_or_id: "s +
                                     e.what());
         }
         info = get_state_row_secondary<block_info>(
             db_view_state.kv_state.view,
   ...
   const std::vector<char>& query_get_abi(wasm_ql::thread_state& thread_state,
                                          const std::vector<char>& contract_kv_prefix,
                                          std::string_view body)
   {
   ...
   EOSIO_REFLECT(transaction_for_get_keys,
                 base ship_protocol::transaction_header,
   ...
   const std::vector<char>& query_get_required_keys(wasm_ql::thread_state& thread_state,
                                                    std::string_view body)
   {
      ...
      {
         throw std::runtime_error("An error occurred deserializing get_required_keys_params: "s +
                                  e.what());
      }
   ...
   EOSIO_REFLECT(send_transaction_params,
                 signatures,
                 compression,
                                                   ...
                                                   bool return_trace_on_except)
   {
      send_transaction_params params;
      {
         ...
         {
            throw std::runtime_error("An error occurred deserializing send_transaction_params: "s +
                                     e.what());
         }
      }
      ...
      results.processed =
          query_send_transaction(thread_state, contract_kv_prefix, trx, snapshot.snapshot(), memory,
                                               ...
                                               bool return_trace_on_except)
   {
      ...
      if (!unpacked.context_free_actions.empty())
         throw std::runtime_error(
             "context_free_actions must be empty");  // todo: is there a case where CFA makes sense?
      ...
      auto stop_time =
          start_time + std::chrono::milliseconds{thread_state.shared->max_exec_time_ms};
   ...
   EOSIO_REFLECT(create_checkpoint_result,
                 path,
                 chain_id,
                 head,
                 head_id,
                                                    ...
                                                    const boost::filesystem::path& dir)
   {
      try
      {
         ...
         create_checkpoint_result result;
         {
            ...
            result.path = tmp_path.string() + ("-head-" + std::to_string(result.head) + "-" +
                                               head_id_json.substr(1, head_id_json.size() - 2));
            ...
            ilog("    head:         ${a} ${b}",
                 ("a", snap.head)("b", eosio::convert_to_json(snap.head_id)));
