...
struct rodeos_error_s
{
   ...
   bool set(const char* m)
   {
      ...
      {
         msg = "error storing error message";
      }
      return false;
   }
};
...
extern "C" void rodeos_destroy_error(rodeos_error* error)
{
   std::unique_ptr<rodeos_error>{error};
}
...
extern "C" void rodeos_destroy(rodeos_context* context)
{
   std::unique_ptr<rodeos_context>{context};
}

extern "C" rodeos_bool rodeos_open_db(rodeos_error* error,
...
extern "C" rodeos_db_partition* rodeos_create_partition(rodeos_error* error,
                                                        rodeos_context* context,
                                                        const char* prefix,
                                                        uint32_t prefix_size)
{
   return handle_exceptions(error, nullptr, [&]() -> rodeos_db_partition* {
      ...
      p->obj = std::make_shared<b1::rodeos::rodeos_db_partition>(
...
extern "C" rodeos_db_snapshot* rodeos_create_snapshot(rodeos_error* error,
...
extern "C" rodeos_bool rodeos_start_block(rodeos_error* error,
...
extern "C" rodeos_bool rodeos_end_block(rodeos_error* error,
                                        rodeos_db_snapshot* snapshot,
                                        const char* data,
                                        uint64_t size,
                                        bool force_write)
...
extern "C" rodeos_bool rodeos_write_block_info(rodeos_error* error,
                                               rodeos_db_snapshot* snapshot,
                                               const char* data,
                                               uint64_t size)
{
...
extern "C" rodeos_bool rodeos_write_deltas(rodeos_error* error,
                                           rodeos_db_snapshot* snapshot,
                                           const char* data,
                                           uint64_t size,
...
extern "C" rodeos_filter* rodeos_create_filter(rodeos_error* error,
                                               uint64_t name,
...
extern "C" void rodeos_destroy_filter(rodeos_filter* filter)
{
   std::unique_ptr<rodeos_filter>{filter};
}

extern "C" rodeos_bool rodeos_run_filter(rodeos_error* error,
                                         rodeos_db_snapshot* snapshot,
                                         ...
                                         void* push_data_arg)
{
   return handle_exceptions(error, false, [&]() {
      ...
      with_result(data, size, [&](auto& result) {
         filter->process(*snapshot, result, {data, data + size},
...
extern "C" rodeos_query_handler* rodeos_create_query_handler(rodeos_error* error,
                                                             rodeos_db_partition* partition,
...
{
   std::unique_ptr<rodeos_query_handler>{handler};
}
