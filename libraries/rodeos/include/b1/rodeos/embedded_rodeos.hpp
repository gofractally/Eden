...
namespace b1::embedded_rodeos
{
   ...
   struct context
   {
      ...
      {
         error.check([&] {
            return rodeos_open_db(error, obj, path, create_if_missing, num_threads, max_open_files);
         });
      }
   };
   ...
   struct partition
   {
      ...
      {
         obj = error.check(
             [&] { return rodeos_create_partition(error, context, prefix, prefix_size); });
      }
      ...
      void write_deltas(const char* data, uint64_t size, F shutdown)
      {
         error.check([&] {
            return rodeos_write_deltas(
                error, obj, data, size,
                [](void* f) -> rodeos_bool { return (*static_cast<F*>(f))(); }, &shutdown);
         });
      }
   };
   ...
   struct filter
   {
      ...
      {
         error.check(
             [&] { return rodeos_run_filter(error, snapshot, obj, data, size, nullptr, nullptr); });
      }
   ...
   struct query_handler
   {
      ...
      query_handler(rodeos_db_partition* partition,
      ...
      {
         obj = error.check([&] {
            return rodeos_create_query_handler(error, partition, max_console_size, wasm_cache_size,
                                               max_exec_time_ms, contract_dir);
         });
      }
      ...
      result query_transaction(rodeos_db_snapshot* snapshot, const char* data, uint64_t size)
      {
         result r;
         error.check([&] {
            return rodeos_query_transaction(error, obj, snapshot, data, size, &r.data, &r.size);
         });
         return r;
      }
   };
