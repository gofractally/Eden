...
namespace b1::rodeos::filter
{
   ...
   struct eosvmoc_tier
   {
      eosvmoc_tier(const boost::filesystem::path& d,
                   const eosio::chain::eosvmoc::config& c,
                   const std::vector<uint8_t>& code,
                   const eosio::chain::digest_type& code_hash)
          : cc(d,
               c,
               [code, code_hash](const eosio::chain::digest_type& id,
                                 uint8_t vm_version) -> std::string_view {
                  if (id == code_hash)
                      ...
                      b1::rodeos::unimplemented_filter_callbacks<callbacks>
   {
      ...
      callbacks(filter::filter_state& filter_state,
                b1::rodeos::chaindb_state& chaindb_state,
                b1::rodeos::db_view_state& db_view_state)
