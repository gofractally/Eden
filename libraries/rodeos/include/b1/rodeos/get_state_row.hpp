...
namespace b1::rodeos
{
   ...
   std::optional<std::pair<std::shared_ptr<const chain_kv::bytes>, T>> get_state_row(
       chain_kv::view& view,
       const K& key)
   {
      ...
      {
         throw std::runtime_error("An error occurred deserializing state: " +
                                  std::string(e.what()));
      }
      return result;
   }
   ...
   template <typename T, typename K>
   std::optional<std::pair<std::shared_ptr<const chain_kv::bytes>, T>> get_state_row_secondary(
       chain_kv::view& view,
       const K& key)
   {
      std::optional<std::pair<std::shared_ptr<const chain_kv::bytes>, T>> result;
