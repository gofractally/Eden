...
namespace b1::rodeos
{
   ...
   using fill_status_sing =
       eosio::kv_singleton<fill_status, eosio::name{"fill.status"}, state_database>;
   ...
   struct block_info_kv : eosio::kv_table<block_info>
   {
      index<uint32_t> primary_index{eosio::name{"primary"}, [](const auto& var) {
                                       return std::visit([](const auto& obj) { return obj.num; },
                                                         *var);
                                    }};

      index<eosio::checksum256> id_index{
          eosio::name{"id"},
          [](const auto& var) { return std::visit([](const auto& obj) { return obj.id; }, *var); }};

      block_info_kv(eosio::kv_environment environment)
          : eosio::kv_table<block_info>{std::move(environment)}
      {
   ...
   struct global_property_kv : eosio::kv_table<global_property>
   {
      ...
      global_property_kv(eosio::kv_environment environment)
          : eosio::kv_table<global_property>{std::move(environment)}
      {
   ...
   struct account_kv : eosio::kv_table<account>
   {
      index<eosio::name> primary_index{eosio::name{"primary"}, [](const auto& var) {
                                          return std::visit(
                                              [](const auto& obj) { return obj.name; }, *var);
                                       }};

      account_kv(eosio::kv_environment environment)
          : eosio::kv_table<account>{std::move(environment)}
      {
   ...
   struct code_kv : eosio::kv_table<code>
   {
      index<std::tuple<const uint8_t&, const uint8_t&, const eosio::checksum256&>> primary_index{
          eosio::name{"primary"}, [](const auto& var) {
             return std::visit(
                 [](const auto& obj) {
                    return std::tie(obj.vm_type, obj.vm_version, obj.code_hash);
                 },
                 *var);
          }};
   ...
   struct contract_table_kv : eosio::kv_table<contract_table>
   {
      index<std::tuple<const eosio::name&, const eosio::name&, const eosio::name&>> primary_index{
          eosio::name{"primary"}, [](const auto& var) {
             return std::visit(
                 [](const auto& obj) { return std::tie(obj.code, obj.table, obj.scope); }, *var);
          }};

      contract_table_kv(eosio::kv_environment environment)
          : eosio::kv_table<contract_table>{std::move(environment)}
      {
   ...
   {
      using PT = typename std::
          tuple<const eosio::name&, const eosio::name&, const eosio::name&, const uint64_t&>;
      index<PT> primary_index{eosio::name{"primary"}, [](const auto& var) {
                                 return std::visit(
                                     [](const auto& obj) {
                                        return std::tie(obj.code, obj.table, obj.scope,
                                                        obj.primary_key);
                                     },
                                     *var);
                              }};
      ...
      contract_row_kv(eosio::kv_environment environment)
          : eosio::kv_table<contract_row>{std::move(environment)}
      {
   ...
   struct contract_index64_kv : eosio::kv_table<contract_index64>
   {
      ...
      using ST = typename std::tuple<const eosio::name&,
                                     const eosio::name&,
                                     const eosio::name&,
                                     const uint64_t&,
                                     const uint64_t&>;
      ...
      {
         init(state_account, eosio::name{"contract.i1"}, state_database, primary_index,
              secondary_index);
      }
   };
   ...
   struct contract_index128_kv : eosio::kv_table<contract_index128>
   {
      ...
      {
         init(state_account, eosio::name{"contract.i2"}, state_database, primary_index,
              secondary_index);
      }
   };
   ...
   struct contract_index_double_kv : eosio::kv_table<contract_index_double>
   {
      ...
      {
         init(state_account, eosio::name{"contract.dbl"}, state_database, primary_index,
              secondary_index);
      }
   };
   ...
   struct contract_index256_kv : eosio::kv_table<contract_index256>
   {
      ...
      {
         init(state_account, eosio::name{"contract.cs"}, state_database, primary_index,
              secondary_index);
      }
   };
   ...
   void store_delta_kv(eosio::kv_environment environment, table_delta_v0& delta, F f)
   {
      for (auto& row : delta.rows)
      {
         ...
         if (row.present)
            environment.kv_set(obj0.database.value, obj0.contract.value, obj0.key.pos,
                               obj0.key.remaining(), obj0.value.pos, obj0.value.remaining());
         else
            environment.kv_erase(obj0.database.value, obj0.contract.value, obj0.key.pos,
                                 obj0.key.remaining());
      }
   }
#endif
