...
namespace b1::rodeos
{
   ...
   enum class kv_it_stat
   {
      ...
      iterator_erased =
          -1,  // The key-value pair that the iterator used to be positioned at was erased
   ...
   struct kv_iterator_rocksdb
   {
      ...
      kv_iterator_rocksdb(uint32_t& num_iterators,
                          chain_kv::view& view,
                          uint64_t contract,
                          const char* prefix,
                          uint32_t size)
      ...
      int32_t kv_it_compare(const kv_iterator_rocksdb& rhs)
      {
         ...
         eosio::check(&view == &r.view && contract == r.contract,
      ...
      kv_it_stat kv_it_next(uint32_t* found_key_size = nullptr,
      ...
      kv_it_stat kv_it_prev(uint32_t* found_key_size = nullptr,
                            uint32_t* found_value_size = nullptr)
      {
         eosio::check(!kv_it.is_erased(), "Iterator to erased element");
         --kv_it;
      ...
      kv_it_stat kv_it_lower_bound(const char* key,
                                   uint32_t size,
                                   uint32_t* found_key_size = nullptr,
                                   uint32_t* found_value_size = nullptr)
      {
      ...
      kv_it_stat kv_it_key(uint32_t offset, char* dest, uint32_t size, uint32_t& actual_size)
      {
         ...
         if (kv)
         {
            if (offset < kv->key.size())
               memcpy(dest, kv->key.data() + offset,
                      std::min((size_t)size, kv->key.size() - offset));
      ...
      kv_it_stat kv_it_value(uint32_t offset, char* dest, uint32_t size, uint32_t& actual_size)
      {
         ...
         if (kv)
         {
            if (offset < kv->value.size())
               memcpy(dest, kv->value.data() + offset,
                      std::min((size_t)size, kv->value.size() - offset));
   ...
   struct kv_context_rocksdb
   {
      ...
      kv_context_rocksdb(chain_kv::database& database,
                         chain_kv::write_session& write_session,
                         std::vector<char> contract_kv_prefix,
                         eosio::name database_id,
                         eosio::name receiver,
          ...
          : database{database},
            ...
            contract_kv_prefix{std::move(contract_kv_prefix)},
      ...
      int64_t kv_set(uint64_t contract,
                     const char* key,
                     uint32_t key_size,
                     const char* value,
                     uint32_t value_size)
      ...
      std::unique_ptr<kv_iterator_rocksdb> kv_it_create(uint64_t contract,
                                                        const char* prefix,
                                                        uint32_t size)
      {
   ...
   struct db_view_state
   {
      ...
      db_view_state(eosio::name receiver,
                    chain_kv::database& database,
                    chain_kv::write_session& write_session,
          ...
          : receiver{receiver},
            ...
            kv_state{database,    write_session, contract_kv_prefix,
                     state_db_id, receiver,      kv_state_limits},
            kv_iterators(1)
      {
      }
      ...
      void reset()
      {
         eosio::check(kv_iterators.size() == kv_destroyed_iterators.size() + 1,
                      "iterators are still alive");
   ...
   struct db_callbacks
   {
      ...
      uint32_t kv_it_create(uint64_t db, uint64_t contract, eosio::vm::span<const char> prefix)
      {
         ...
         else
         {
            ...
            eosio::check(derived().get_db_view_state().kv_iterators.size() <= 0xFFFFFFFFu,
                         "Too many iterators");
         ...
         derived().get_db_view_state().kv_iterators[itr] =
             kdb.kv_it_create(contract, prefix.data(), prefix.size());
         return itr;
      }
      ...
      int32_t kv_it_status(uint32_t itr)
      {
         ...
         return static_cast<int32_t>(
             derived().get_db_view_state().kv_iterators[itr]->kv_it_status());
      }
      ...
      {
         kv_check_iterator(itr);
         return derived().get_db_view_state().kv_iterators[itr]->kv_it_key_compare(key.data(),
                                                                                   key.size());
      }
      ...
      int32_t kv_it_move_to_end(uint32_t itr)
      {
         kv_check_iterator(itr);
         return static_cast<int32_t>(
             derived().get_db_view_state().kv_iterators[itr]->kv_it_move_to_end());
      }
      ...
      int32_t kv_it_next(uint32_t itr, uint32_t* found_key_size, uint32_t* found_value_size)
      {
         ...
         return static_cast<int32_t>(derived().get_db_view_state().kv_iterators[itr]->kv_it_next(
             found_key_size, found_value_size));
      }
      ...
      int32_t kv_it_lower_bound(uint32_t itr,
                                eosio::vm::span<const char> key,
                                ...
                                uint32_t* found_value_size)
      {
         kv_check_iterator(itr);
         return static_cast<int32_t>(
             derived().get_db_view_state().kv_iterators[itr]->kv_it_lower_bound(
                 key.data(), key.size(), found_key_size, found_value_size));
      }

      int32_t kv_it_key(uint32_t itr,
                        uint32_t offset,
                        eosio::vm::span<char> dest,
                        uint32_t* actual_size)
      {
         kv_check_iterator(itr);
         return static_cast<int32_t>(derived().get_db_view_state().kv_iterators[itr]->kv_it_key(
             offset, dest.data(), dest.size(), *actual_size));
      }

      int32_t kv_it_value(uint32_t itr,
                          uint32_t offset,
                          eosio::vm::span<char> dest,
                          uint32_t* actual_size)
      {
   ...
   class kv_environment : public db_callbacks<kv_environment>
   {
     public:
      ...
      void kv_set(uint64_t db,
                  uint64_t contract,
                  const std::vector<char>& k,
      ...
      uint32_t kv_it_create(uint64_t db,
                            uint64_t contract,
                            const char* prefix,
