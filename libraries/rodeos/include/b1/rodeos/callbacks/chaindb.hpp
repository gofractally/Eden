...
namespace b1::rodeos
{
   ...
   class iterator_cache_base
   {
     protected:
      ...
      struct table_key
      {
         ...
         friend bool operator<(const table_key& a, const table_key& b)
         {
      ...
      struct table_index_key
      {
         ...
         friend bool operator<(const table_index_key& a, const table_index_key& b)
         {
            return a.order() < b.order();
         }
      };
      ...
      int32_t get_iterator_impl(int32_t table_index,
                                const std::optional<T>& key,
                                chain_kv::view::iterator&& view_it,
                                const M& key_to_iterator_index,
                                bool view_it_is_secondary,
                                F1 match,
                                F2 get_key)
      {
         iterator* it;
   ...
   class iterator_cache : public iterator_cache_base<iterator_cache, chaindb_primary_iterator>
   {
     public:
      ...
      int32_t get_iterator(int32_t table_index,
                           uint64_t key,
                           chain_kv::view::iterator&& view_it,
                           bool require_match_primary = false)
      {
         return get_iterator_impl<contract_row>(
             table_index, std::optional{key}, std::move(view_it), primary_key_to_iterator_index,
             false,
      ...
      auto create_view_it(const iterator& it, bool exists)
      {
         ...
         chain_kv::view::iterator view_it{
             ...
             chain_kv::to_slice(eosio::convert_to_key(
                 std::make_tuple((uint8_t)0x01, eosio::name{"contract.row"}, eosio::name{"primary"},
      ...
      int db_next_i64(int itr, uint64_t& primary)
      {
         return next_impl<contract_row>(
             itr, primary, false,
             [this](auto&&... args) {
                return create_view_it(std::forward<decltype(args)>(args)...);
             },
      ...
      int32_t lower_bound(uint64_t code,
                          uint64_t scope,
                          uint64_t table,
                          uint64_t key,
                          bool require_exact)
   ...
   struct chaindb_secondary_iterator : chaindb_iterator_base
   {
      using secondary_type =
          std::decay_t<decltype(std::get<0>(std::declval<Ship_index_type>()).secondary_key)>;

      secondary_type secondary = {};
   };

   template <typename Ship_index_type>
   class secondary_iterator_cache
       : public iterator_cache_base<secondary_iterator_cache<Ship_index_type>,
                                    chaindb_secondary_iterator<Ship_index_type>>
   {
     public:
      using base = iterator_cache_base<secondary_iterator_cache<Ship_index_type>,
                                       chaindb_secondary_iterator<Ship_index_type>>;
      using secondary_type =
          std::decay_t<decltype(std::get<0>(std::declval<Ship_index_type>()).secondary_key)>;
      ...
      struct whole_key
      {
         ...
         friend bool operator<(const whole_key& a, const whole_key& b)
         {
            return a.order() < b.order();
         }
      };

      std::map<typename base::template table_index_key<whole_key>, int32_t>
          secondary_to_iterator_index;

      secondary_iterator_cache(chain_kv::view& view, eosio::name table_name)
          : base{view}, table_name{table_name}
      {
      }
      ...
      std::pair<size_t, iterator*> create_iterator(int32_t table_index, T& record)
      {
         ...
         secondary_to_iterator_index[{table_index, {record.secondary_key, record.primary_key}}] =
             pos;
         return {pos, it};
      }
      ...
      int32_t get_secondary_iterator(int32_t table_index,
                                     chain_kv::view::iterator&& view_it,
                                     const secondary_type& search_secondary,
                                     secondary_type& found_secondary,
                                     std::optional<uint64_t> search_primary,
                                     uint64_t& found_primary,
                                     bool require_match_secondary)
      {
         auto itr = base::template get_iterator_impl<Ship_index_type>(
             table_index,
             search_primary ? std::optional{whole_key{search_secondary, *search_primary}}
                            : std::nullopt,
             std::move(view_it), secondary_to_iterator_index, true,
             [&](auto& record) {
                return !require_match_secondary || record.secondary_key == search_secondary;
             },
             [&](auto& record) {
      ...
      auto create_view_it(const iterator& it, bool exists)
      {
         ...
         chain_kv::view::iterator view_it{base::view, state_account.value,
                                          chain_kv::to_slice(eosio::convert_to_key(std::make_tuple(
                                              (uint8_t)0x01, table_name, eosio::name{"secondary"},
                                              table_key.code, table_key.table, table_key.scope)))};
         if (exists)
            view_it.lower_bound(eosio::convert_to_key(
                std::make_tuple((uint8_t)0x01, table_name, eosio::name{"secondary"}, table_key.code,
                                table_key.table, table_key.scope, it.secondary, it.primary)));
         return view_it;
      }
         ...
         return base::template next_impl<Ship_index_type>(
             ...
             [this](int32_t table_index, auto&& record, chain_kv::view::iterator&& view_it) {
                return get_secondary_iterator(table_index, std::move(view_it), record.secondary_key,
                                              record.secondary_key, record.primary_key,
                          ...
                          bool require_exact)
      {
         ...
         it.lower_bound(eosio::convert_to_key(std::make_tuple(
             (uint8_t)0x01, table_name, eosio::name{"secondary"}, code, table, scope, secondary)));
         return get_secondary_iterator(table_index, std::move(it), secondary, secondary, 0, primary,
                                       require_exact);
      }

      int32_t upper_bound(uint64_t code,
                          uint64_t scope,
                          uint64_t table,
                          secondary_type& secondary,
                          uint64_t& primary)
      {
         ...
         chain_kv::view::iterator it{
             ...
             chain_kv::to_slice(eosio::convert_to_key(std::make_tuple(
                 (uint8_t)0x01, table_name, eosio::name{"secondary"}, code, table, scope)))};
         it.lower_bound(eosio::convert_to_key(
             std::make_tuple((uint8_t)0x01, table_name, eosio::name{"secondary"}, code, table,
                             scope, secondary, ~uint64_t(0), uint8_t(0))));
         return get_secondary_iterator(table_index, std::move(it), secondary, secondary,
                                       std::nullopt, primary, false);
      }

      int32_t find_primary(uint64_t code,
                           uint64_t scope,
                           uint64_t table,
                           uint64_t primary,
                           secondary_type& secondary)
   ...
   struct chaindb_callbacks
   {
      ...
      iterator_cache& get_iterator_cache()
      {
         ...
         if (!chaindb_state.iterator_cache)
            chaindb_state.iterator_cache =
                std::make_unique<iterator_cache>(derived().get_db_view_state().kv_state.view);
...
#define IMPL_SECONDARY(NAME, TYPE, TABLE_NAME)                                                 \
   ...
   void db_##NAME##_remove(int32_t iterator)                                                   \
   {                                                                                           \
      throw std::runtime_error("unimplemented: db_" #NAME "_remove");                          \
   }                                                                                           \
   ...
   int32_t db_##NAME##_find_primary(uint64_t code, uint64_t scope, uint64_t table,             \
                                    legacy_ptr<TYPE> secondary, uint64_t primary)              \
   {                                                                                           \
      return get_##NAME().find_primary(code, scope, table, primary, *secondary);               \
   }                                                                                           \
   int32_t db_##NAME##_find_secondary(uint64_t code, uint64_t scope, uint64_t table,           \
                                      legacy_ptr<const TYPE> secondary,                        \
   ...
   int32_t db_##NAME##_lowerbound(uint64_t code, uint64_t scope, uint64_t table,               \
                                  legacy_ptr<TYPE> secondary, legacy_ptr<uint64_t> primary)    \
   {                                                                                           \
      return get_##NAME().lower_bound(code, scope, table, *secondary, *primary, false);        \
   }                                                                                           \
   int32_t db_##NAME##_upperbound(uint64_t code, uint64_t scope, uint64_t table,               \
                                  legacy_ptr<TYPE> secondary, legacy_ptr<uint64_t> primary)    \
   {                                                                                           \
...
#undef IMPL_SECONDARY
      ...
      int32_t db_idx256_store(uint64_t scope,
                              uint64_t table,
                              uint64_t payer,
                              uint64_t id,
      ...
      void db_idx256_update(int32_t iterator,
                            uint64_t payer,
                            legacy_span<const __uint128_t> secondary)
      {
      ...
      {
         throw std::runtime_error("unimplemented: db_idx256_remove");
      }
      ...
      int32_t db_idx256_find_primary(uint64_t code,
                                     uint64_t scope,
                                     uint64_t table,
                                     legacy_span<__uint128_t> arr_128,
                                     uint64_t primary)
      {
      ...
      int32_t db_idx256_find_secondary(uint64_t code,
                                       uint64_t scope,
                                       uint64_t table,
                                       legacy_span<const __uint128_t> secondary,
                                       legacy_ptr<uint64_t> primary)
      ...
      int32_t db_idx256_lowerbound(uint64_t code,
                                   uint64_t scope,
                                   uint64_t table,
                                   legacy_span<__uint128_t> secondary,
                                   legacy_ptr<uint64_t> primary)
      {
         auto cs = read_cs(secondary);
      ...
      int32_t db_idx256_upperbound(uint64_t code,
                                   uint64_t scope,
                                   uint64_t table,
                                   legacy_span<__uint128_t> secondary,
                                   legacy_ptr<uint64_t> primary)
      ...
      {
         return get_idx256().end(code, scope, table);
      }
