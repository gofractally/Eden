...
namespace b1::rodeos
{
   ...
   struct type_converter : eosio::vm::type_converter<Host, Execution_Interface>
   {
      ...
      EOS_VM_FROM_WASM(memcpy_params,
                       (eosio::vm::wasm_ptr_t dst,
                        eosio::vm::wasm_ptr_t src,
                        eosio::vm::wasm_size_t size))
      {
      ...
      EOS_VM_FROM_WASM(memcmp_params,
                       (eosio::vm::wasm_ptr_t lhs,
                        eosio::vm::wasm_ptr_t rhs,
                        eosio::vm::wasm_size_t size))
      {
      ...
      EOS_VM_FROM_WASM(memset_params,
                       (eosio::vm::wasm_ptr_t dst, int32_t val, eosio::vm::wasm_size_t size))
      {
         auto d = this->template validate_pointer<char>(dst, size);
         this->template validate_pointer<char>(dst, 1);
         return {d, val, size};
      ...
      template <typename T>
      auto from_wasm(uint32_t ptr) const
          -> std::enable_if_t<std::is_pointer_v<T>, eosio::vm::argument_proxy<T>>
      {
      ...
      auto from_wasm(eosio::vm::wasm_ptr_t ptr, eosio::vm::tag<T> = {}) const
          -> std::enable_if_t<eosio::vm::is_argument_proxy_type_v<T> &&
                                  std::is_pointer_v<typename T::proxy_type>,
                              T>
      {
   ...
   using registered_host_functions =
       eosio::vm::registered_host_functions<Cls,
   ...
   struct eos_vm_oc_type_converter
       : public type_converter<Cls, eosio::chain::eosvmoc::eos_vm_oc_execution_interface>
   {
   ...
   {
      constexpr auto fn =
          eosio::chain::eosvmoc::create_function<F, Cls, eos_vm_oc_type_converter<Cls>,
                                                 Preconditions>();
      get_intrinsic_map().insert({n.c_str(),
                                  {reinterpret_cast<void*>(fn),
                                   ::boost::hana::index_if(eosio::chain::eosvmoc::intrinsic_table,
                                                           ::boost::hana::equal.to(n))
                                       .value()}});
   }
#endif
   ...
   {
#ifdef EOSIO_EOS_VM_OC_RUNTIME_ENABLED
      register_eosvm_oc<F, typename Rft::host_type_t, std::tuple<>>(BOOST_HANA_STRING("env.") +
                                                                    fn_name);
#endif
