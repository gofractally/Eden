...
namespace b1::rodeos
{
   ...
   struct console_callbacks
   {
      ...
      void append_console(const char* str, uint32_t len)
      {
         ...
         state.console.append(str,
                              std::min(size_t(len), state.max_console_size - state.console.size()));
      }
      ...
      void printui128(legacy_ptr<const unsigned __int128> p)
      {
         append_console("{printui128 unimplemented}");
      }
      ...
      {
         auto& state = derived().get_state();
         for (uint32_t i = 0; i < data.size() && state.console.size() + 2 < state.max_console_size;
              ++i)
         {
