...
namespace b1::rodeos::wasm_ql
{
     ...
     public:
      ...
      handle get_state()
      {
         ...
         if (states.empty())
         {
            ...
            {
               elog("vm::exception creating thread_state: ${w}: ${d}",
                    ("w", e.what())("d", e.detail()));
   ...
   eosio::ship_protocol::transaction_trace_v0 query_send_transaction(
       wasm_ql::thread_state& thread_state,
       const std::vector<char>& contract_kv_prefix,
       const eosio::ship_protocol::packed_transaction& trx,
