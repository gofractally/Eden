...
namespace b1::rodeos
{
   ...
   rodeos_db_snapshot::rodeos_db_snapshot(std::shared_ptr<rodeos_db_partition> partition,
                                          bool persistent)
       : partition{std::move(partition)}, db{this->partition->db}
   {
      ...
      db_view_state view_state{state_account, *db, *write_session,
   ...
   void rodeos_db_snapshot::start_block(const get_blocks_result_base& result)
   {
      ...
      if (result.this_block->block_num <= head)
      {
         ilog("switch forks at block ${b}; database contains revisions ${f} - ${h}",
              ("b", result.this_block->block_num)("f", undo_stack->first_revision())(
                  "h", undo_stack->revision()));
      ...
      if (head_id != eosio::checksum256{} &&
          (!result.prev_block || result.prev_block->block_id != head_id))
   ...
   void rodeos_db_snapshot::write_block_info(uint32_t block_num,
   ...
   void rodeos_db_snapshot::write_block_info(const ship_protocol::get_blocks_result_v1& result)
   {
      ...
      const signed_block_header& header =
          std::visit([](const auto& blk) { return static_cast<const signed_block_header&>(blk); },
                     *result.block);
      ...
      fc_add_tag(blk_span, "block_id", to_string(result.this_block->block_id));
      fc_add_tag(blk_span, "block_num", block_num);
      fc_add_tag(blk_span, "block_time",
                 eosio::microseconds_to_str(header.timestamp.to_time_point().elapsed.count()));

      write_block_info(block_num, result.this_block->block_id, header);
   }

   void rodeos_db_snapshot::write_deltas(
       uint32_t block_num,
       eosio::opaque<std::vector<ship_protocol::table_delta>> deltas,
         ...
         store_delta({view_state}, delta_v0, head == 0, [&]() {
            if (delta_v0.rows.size() > 10000 && !(num_processed % 10000))
            {
               ...
               ilog("block ${b} ${t} ${n} of ${r}",
                    ("b", block_num)("t", delta_v0.name)("n", num_processed)("r",
                                                                             delta_v0.rows.size()));
               if (head == 0)
               {
                  end_write(false);
                                         ...
                                         std::function<bool()> shutdown)
   {
      ...
      uint32_t block_num = result.this_block->block_num;
      write_deltas(block_num,
                   eosio::opaque<std::vector<ship_protocol::table_delta>>(*result.deltas),
                   shutdown);
   }
   ...
   rodeos_filter::rodeos_filter(eosio::name name,
                                const std::string& wasm_filename,
                                bool profile
      ...
      if (eosvmoc_enable)
      {
         try
         {
            filter_state->eosvmoc_tierup.emplace(
                ...
                eosio::chain::digest_type::hash(reinterpret_cast<const char*>(code.data()),
                                                code.size()));
         }
         FC_LOG_AND_RETHROW();
      }
   ...
   void rodeos_filter::process(
       rodeos_db_snapshot& snapshot,
       const ship_protocol::get_blocks_result_base& result,
       ...
       const std::function<void(const char* data, uint64_t size)>& push_data)
   {
      ...
      db_view_state view_state{name, *snapshot.db, *snapshot.write_session,
      ...
      {
         const auto* code = filter_state->eosvmoc_tierup->cc.get_descriptor_for_code(
             filter_state->eosvmoc_tierup->hash, 0);
         if (code)
         {
            eosio::chain::eosvmoc::timer_base timer;
            filter_state->eosvmoc_tierup->exec.execute(*code, filter_state->eosvmoc_tierup->mem,
                                                       &cb, 251, 65536, &timer, 0, 0, 0);
            return;
         }
      }
#endif
   ...
   rodeos_query_handler::rodeos_query_handler(
       ...
       : partition{partition},
         ...
         state_cache{std::make_shared<wasm_ql::thread_state_cache>(this->shared_state)}
   {
   }
