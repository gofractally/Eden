...
namespace eosio
{
   namespace detail
   {
      ...
      {
         if (base <= 1)
            check(false, "base must be at least 2");
         T prior = 1;
         T result = prior;
   ...
   inline constexpr auto powers_of_base =
       detail::generate_array<detail::largest_power<T, Base>::exponent + 1>(
           detail::pow_generator<T, Base>);
