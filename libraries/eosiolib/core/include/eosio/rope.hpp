   ...
   class rope
   {
     private:
      ...
      static void c_str(char* buffer, const rope_node* r, size_t& off)
      {
         ...
         if (r)
         {
            std::visit([](const auto& n) { n.temp = nullptr; }, *r);
         }
         while (r)
         {
            if (going_down)
            {
               std::visit(overloaded{[&](const concat_t& c) {
                                        if (c.left)
                                        {
                                           c.temp_left = true;
                                           std::visit([&](const auto& n) { n.temp = r; }, *c.left);
                                           going_down = true;
                                           r = c.left;
                                        }
                                        else if (c.right)
                                        {
                                           c.temp_left = false;
                                           std::visit([&](const auto& n) { n.temp = r; }, *c.right);
                                           going_down = true;
                                           r = c.right;
                                        }
                                        else
     ...
     public:
      ...
      constexpr char operator[](size_t index) { return at(index); }
      ...
      void print() const { eosio::print(c_str()); }
