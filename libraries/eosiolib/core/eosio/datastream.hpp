...
namespace eosio
{
   ...
   class datastream
   {
     public:
       ...
       *
       * @details Construct a new datastream object given the size of the buffer and start position
       * of the buffer
      ...
      {
         eosio::check(size_t(_end - _pos) >= (size_t)s,
                      "datastream attempted to read past the end");
         memcpy(d, _pos, s);
         _pos += s;
         return true;
      }
   ...
   namespace _datastream_detail
   {
      ...
      namespace operator_detection
      {
         ...
         template <
             typename DataStream,
             typename T,
             std::enable_if_t<_datastream_detail::is_datastream<DataStream>::value>* = nullptr>
         ...
         using require_specialized_right_shift =
             std::void_t<decltype(std::declval<DataStream&>() >> std::declval<T&>())>;
         template <typename DataStream, typename T>
         using require_specialized_left_shift =
             std::void_t<decltype(std::declval<DataStream&>() << std::declval<const T&>())>;
      }  // namespace operator_detection
      // Going through enable_if/is_detected is necessary for reasons that I don't entirely
      // understand.
      template <typename DataStream, typename T>
      using require_specialized_right_shift = std::enable_if_t<
          std::experimental::
              is_detected_v<operator_detection::require_specialized_right_shift, DataStream, T>>;
      template <typename DataStream, typename T>
      using require_specialized_left_shift = std::enable_if_t<
          std::experimental::
              is_detected_v<operator_detection::require_specialized_left_shift, DataStream, T>>;
   }  // namespace _datastream_detail
   ...
   template <
       typename DataStream,
       typename T,
       _datastream_detail::require_specialized_left_shift<datastream<DataStream>, T>* = nullptr>
   ...
   template <
       typename DataStream,
       typename T,
       _datastream_detail::require_specialized_right_shift<datastream<DataStream>, T>* = nullptr>
