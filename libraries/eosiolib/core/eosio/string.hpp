...
namespace eosio
{
   class string
   {
     public:
      ...
      char& back() { return at(_size - 1); }

      const char back() const { return at(_size - 1); }

      char* data() { return begin(); }

      const char* data() const { return cbegin(); }
      ...
      const char* cend() const { return cbegin() + _size; }
      ...
      void reserve(const size_t n)
      {
         if (_capacity < n)
         {
            ...
            const char* tmp{(is_literal()) ? std::get<const char*>(_begin)
                                           : std::get<uptr>(_begin).get()};
      ...
      void swap(string& str) { std::swap(*this, str); }
      ...
      string substr(size_t pos = 0, size_t len = npos) const { return string(*this, pos, len); }
      ...
      string& insert(const size_t pos, const char* str) { return insert(pos, str, strlen(str)); }
      ...
      string& insert(const size_t pos, const char* str, const size_t len)
      {
         ...
         else
         {
            if (is_literal())
               clone(_size, _capacity, std::get<const char*>(_begin));
            _size += len;
            memmove(std::get<uptr>(_begin).get() + pos + len, std::get<uptr>(_begin).get() + pos,
                    _size - pos);
   ...
   {
      const char* beg_lhs{lhs.cbegin()};
      const char* end_lhs{lhs.cend()};
      const char* beg_rhs{rhs.cbegin()};
      const char* end_rhs{rhs.cend()};
