...
namespace eosio
{
    ...
    *
    *  @brief Writes number x 10^(-num_decimal_places) (optionally negative) as a string to the
    ...
    *  @param number - The number to print before shifting the decimal point to the left by
    * num_decimal_places.
    ...
    *  @return char* - Just past the end of the last character that would be written assuming
    * dry_run == false and end was large enough to provide sufficient space. (Meaning only applies
    * if returned pointer >= begin.)
    *  @post If the output string fits within the range [begin, end), the range [begin, returned
    * pointer) contains the string representation of the number. Nothing is written if dry_run ==
    * true or returned pointer > end (insufficient space) or if returned pointer < begin (overflow
                       ...
                       bool negative)
   {
      ...
      uint8_t num_digits = (std::upper_bound(powers_of_ten.begin(), powers_of_ten.end(), number) -
                            powers_of_ten.begin());  // num_digits == 0 iff number == 0
      // 0 <= num_digits <= 20
      ...
      if (num_decimal_places > 0)
         ++characters_needed;  // space for decimal point
