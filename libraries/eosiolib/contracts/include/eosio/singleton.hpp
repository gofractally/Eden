...
namespace eosio
{
   ...
   class singleton
   {
     ...
     public:
       ...
       */
      bool exists() { return _t.find(pk_value) != _t.end(); }

      /**
       * Get the value stored inside the singleton table. Will throw an exception if it doesn't
       * exist
       *
      ...
      /**
       * Get the value stored inside the singleton table. If it doesn't exist, it will create a new
       * one with the specified default value
       *
       * @param bill_to_account - The account to bill for the newly created data if the data doesn't
       * exist
