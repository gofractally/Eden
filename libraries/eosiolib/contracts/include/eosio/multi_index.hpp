...
namespace eosio
{
   ...
   struct const_mem_fun
   {
      ...
      Type operator()(const std::reference_wrapper<Class>& x) const { return operator()(x.get()); }
   };
...
#define WRAP_SECONDARY_SIMPLE_TYPE(IDX, TYPE)                                                      \
   ...
   {                                                                                               \
      ...
      static int32_t db_idx_store(uint64_t scope,                                                  \
                                  uint64_t table,                                                  \
                                  uint64_t payer,                                                  \
                                  uint64_t id,                                                     \
      ...
      static int32_t db_idx_find_primary(uint64_t code,                                            \
                                         uint64_t scope,                                           \
                                         uint64_t table,                                           \
                                         uint64_t primary,                                         \
                                         TYPE& secondary)                                          \
      {                                                                                            \
         return internal_use_do_not_use::db_##IDX##_find_primary(code, scope, table, &secondary,   \
                                                                 primary);                         \
      }                                                                                            \
      static int32_t db_idx_find_secondary(uint64_t code,                                          \
                                           uint64_t scope,                                         \
                                           uint64_t table,                                         \
                                           const TYPE& secondary,                                  \
                                           uint64_t& primary)                                      \
      {                                                                                            \
         return internal_use_do_not_use::db_##IDX##_find_secondary(code, scope, table, &secondary, \
                                                                   &primary);                      \
      }                                                                                            \
      static int32_t db_idx_lowerbound(uint64_t code,                                              \
                                       uint64_t scope,                                             \
                                       uint64_t table,                                             \
                                       TYPE& secondary,                                            \
                                       uint64_t& primary)                                          \
      {                                                                                            \
         return internal_use_do_not_use::db_##IDX##_lowerbound(code, scope, table, &secondary,     \
                                                               &primary);                          \
      }                                                                                            \
      static int32_t db_idx_upperbound(uint64_t code,                                              \
                                       uint64_t scope,                                             \
                                       uint64_t table,                                             \
                                       TYPE& secondary,                                            \
                                       uint64_t& primary)                                          \
      {                                                                                            \
         return internal_use_do_not_use::db_##IDX##_upperbound(code, scope, table, &secondary,     \
                                                               &primary);                          \
      }                                                                                            \
   };
...
#define WRAP_SECONDARY_ARRAY_TYPE(IDX, TYPE, ELEMTYPE)                                             \
   ...
   {                                                                                               \
      ...
      {                                                                                            \
         ...
         return internal_use_do_not_use::db_##IDX##_store(scope, table, payer, id, arr.data(),     \
                                                          arr.size());                             \
      }                                                                                            \
      static void db_idx_update(int32_t iterator, uint64_t payer, const TYPE& secondary)           \
      {                                                                                            \
      ...
      {                                                                                            \
         ...
         int32_t result = internal_use_do_not_use::db_##IDX##_find_primary(                        \
             code, scope, table, arr.data(), arr.size(), primary);                                 \
         secondary = TYPE(arr);                                                                    \
         return result;                                                                            \
      }                                                                                            \
      static int32_t db_idx_find_secondary(uint64_t code,                                          \
      ...
      {                                                                                            \
         auto arr = secondary.extract_as_word_array<ELEMTYPE>();                                   \
         return internal_use_do_not_use::db_##IDX##_find_secondary(code, scope, table, arr.data(), \
                                                                   arr.size(), &primary);          \
      }                                                                                            \
      static int32_t db_idx_lowerbound(uint64_t code,                                              \
                                       uint64_t scope,                                             \
      ...
      {                                                                                            \
         auto arr = secondary.extract_as_word_array<ELEMTYPE>();                                   \
         int32_t result = internal_use_do_not_use::db_##IDX##_lowerbound(                          \
             code, scope, table, arr.data(), arr.size(), &primary);                                \
         secondary = TYPE(arr);                                                                    \
         return result;                                                                            \
      }                                                                                            \
      ...
      {                                                                                            \
         auto arr = secondary.extract_as_word_array<ELEMTYPE>();                                   \
         int32_t result = internal_use_do_not_use::db_##IDX##_upperbound(                          \
             code, scope, table, arr.data(), arr.size(), &primary);                                \
         secondary = TYPE(arr);                                                                    \
         return result;                                                                            \
      }                                                                                            \
...
#define MAKE_TRAITS_FOR_ARITHMETIC_SECONDARY_KEY(TYPE)                                    \
   ...
   {                                                                                      \
      static_assert(std::numeric_limits<TYPE>::is_specialized,                            \
                    "TYPE does not have specialized numeric_limits");                     \
   ...
   /**
    *  The indexed_by struct is used to instantiate the indices for the Multi-Index table. In EOSIO,
    * up to 16 secondary indices can be specified.
    *
    *  @ingroup multiindex
    *  @tparam IndexName - is the name of the index. The name must be provided as an EOSIO base32
    * encoded 64-bit integer and must conform to the EOSIO naming requirements of a maximum of 13
    * characters, the first twelve from the lowercase characters a-z, digits 1-5, and ".", and if
    * there is a 13th character, it is restricted to lowercase characters a-p and ".".
    *  @tparam Extractor - is a function call operator that takes a const reference to the table
    * object type and returns either a secondary key type or a reference to a secondary key type. It
    * is recommended to use the `eosio::const_mem_fun` template, which is a type alias to the
    * `boost::multi_index::const_mem_fun`. See the documentation for the Boost `const_mem_fun` key
    ...
    *     };
    *    public:
    *      mycontract(name receiver, name code, datastream<const char*> ds):contract(receiver, code,
    * ds){} void myaction() { auto code = _self; auto scope = _self; multi_index<"mytable"_n,
    * record, indexed_by< "bysecondary"_n, const_mem_fun<record, uint128_t, &record::get_secondary>
    * > > table( code, scope);
    *      }
    *  }
    ...
    *  @details EOSIO Multi-Index API provides a C++ interface to the EOSIO database. It is
    * patterned after Boost Multi Index Container. EOSIO Multi-Index table requires exactly a
    * uint64_t primary key. For the table to be able to retrieve the primary key, the object stored
    * inside the table is required to have a const member function called primary_key() that returns
    * uint64_t. EOSIO Multi-Index table also supports up to 16 secondary indices. The type of the
    * secondary indices could be any of:
    *  - uint64_t
    *  - uint128_t
    *  - double
    *  - long double
    ...
    *      mycontract(name receiver, name code, datastream<const char*> ds):contract(receiver, code,
    * ds){} void myaction() { auto code = _self; auto scope = _self; multi_index<"mytable"_n,
    * record, indexed_by< "bysecondary1"_n, const_mem_fun<record, uint64_t,
    * &record::get_secondary_1> >, indexed_by< "bysecondary2"_n, const_mem_fun<record, uint128_t,
    * &record::get_secondary_2> >, indexed_by< "bysecondary3"_n, const_mem_fun<record, checksum256,
    * &record::get_secondary_3> >, indexed_by< "bysecondary4"_n, const_mem_fun<record, double,
    * &record::get_secondary_4> >, indexed_by< "bysecondary5"_n, const_mem_fun<record, long double,
    * &record::get_secondary_5> > > table( code, scope);
    *      }
    *  }
    *  EOSIO_DISPATCH( mycontract, (myaction) )
    *  @endcode
   ...
   class multi_index
   {
     private:
      ...
      {
         // Limit table names to 12 characters so that the last character (4 bits) can be used to
         // distinguish between the secondary indices.
      ...
      {
         no_available_primary_key = static_cast<uint64_t>(
             -2),  // Must be the smallest uint64_t value compared to all other tags
      ...
      struct item : public T
      {
         ...
         item(const multi_index* idx, Constructor&& c) : __idx(idx)
         {
            c(*this);
         }
      ...
      struct index
      {
        public:
         ...
         static_assert(validate_index_name(name(IndexName)),
         ...
         enum constants
         {
            ...
            index_table_name =
                ...
                (Number &
                 0x000000000000000FULL)  // Assuming no more than 16 secondary indices are allowed
         };
           ...
           public:
            ...
            const_iterator& operator++()
            {
               ...
               if (_item->__iters[Number] == -1)
               {
                  ...
                  auto idxitr =
                      secondary_index_db_functions<secondary_key_type>::db_idx_find_primary(
               ...
               uint64_t next_pk = 0;
               auto next_itr = secondary_index_db_functions<secondary_key_type>::db_idx_next(
                   _item->__iters[Number], &next_pk);
               if (next_itr < 0)
               {
                  _item = nullptr;
                  return *this;
         ...
         const_iterator find(secondary_key_type&& secondary) const { return find(secondary); }
         ...
         const_iterator require_find(secondary_key_type&& secondary,
                                     const char* error_msg = "unable to find secondary key") const
         {
         ...
         const T& get(secondary_key_type&& secondary,
                      const char* error_msg = "unable to find secondary key") const
         {
            return get(secondary, error_msg);
         }

         // Gets the object with the smallest primary key in the case where the secondary key is not
         // unique.
         ...
         const_iterator iterator_to(const T& obj)
         {
            ...
            eosio::check(objitem.__idx == _multidx,
            ...
            if (objitem.__iters[Number] == -1)
            {
               ...
               auto idxitr = secondary_index_db_functions<secondary_key_type>::db_idx_find_primary(
                   get_code().value, get_scope(), name(), objitem.primary_key(),
                   temp_secondary_key);
      ...
      static constexpr auto transform_indices()
      {
         ...
         return hana::transform(indices_input_type(), [&](auto&& idx) {
            ...
            return hana::make_tuple(
                hana::type_c<
                    index<eosio::name::raw(static_cast<uint64_t>(idx_type::index_name)),
      ...
      const item& load_object_by_primary_iterator(int32_t itr) const
      {
         ...
         auto itr2 = std::find_if(_items_vector.rbegin(), _items_vector.rend(),
                                  [&](const item_ptr& ptr) { return ptr._primary_itr == itr; });
         ...
         // exceptions
         void* buffer =
             max_stack_buffer_size < size_t(size) ? malloc(size_t(size)) : alloca(size_t(size));
     ...
     public:
       ...
       *  @post Secondary indices are updated to refer to the newly added object. If the secondary
       * index tables do not exist, they are created.
       *  @post The payer is charged for the storage usage of the new object and, if the table (and
       * secondary index tables) must be created, for the overhead of the table creation.
       *
       *  Notes
       *  The `eosio::multi_index` template has template parameters `<name::raw TableName, typename
       * T, typename... Indices>`, where:
       *  - `TableName` is the name of the table, maximum 12 characters long, characters in the name
       * from the set of lowercase letters, digits 1 to 5, and the "." (period) character and is
       * converted to a eosio::raw - which wraps uint64_t;
       ...
       *  - Each must have a function call operator that takes a const reference to the table object
       * type and returns either a secondary key type or a reference to a secondary key type
       ...
       *  @ingroup multiindex
       *
       *  @return Scope id of the Scope within the Code of the Current Receiver under which the
       * desired Primary Table instance can be found.
       *
       *  Example:
       *
       *  @code
      ...
      struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T>
      {
         ...
         const_iterator& operator--()
         {
            ...
            if (!_item)
            {
               auto ei = internal_use_do_not_use::db_end_i64(_multidx->get_code().value,
                                                             _multidx->get_scope(),
        ...
        private:
         const_iterator(const multi_index* mi, const item* i = nullptr) : _multidx(mi), _item(i) {}
      ...
      /**
       *  Returns an iterator pointing to the object_type with the lowest primary key value in the
       * Multi-Index table.
       *  @ingroup multiindex
       *
       *  @return An iterator pointing to the object_type with the lowest primary key value in the
       * Multi-Index table.
       *
       *  Example:
       *
       ...
       *  @endcode
       */
      const_iterator cbegin() const { return lower_bound(std::numeric_limits<uint64_t>::lowest()); }
      ...
      /**
       *  Returns an iterator referring to the `past-the-end` element in the multi index container.
      ...
      /**
       *  Returns a reverse iterator pointing to the `object_type` with the highest primary key
       * value in the Multi-Index table.
       *  @ingroup multiindex
       *
       *  @return A reverse iterator pointing to the `object_type` with the highest primary key
       * value in the Multi-Index table.
       *
       *  Example:
       *
       ...
       *        itr++;
       *        eosio::check(itr->account_name == name("brendan"), "Lock arf, Incorrect Second Last
       * Record");
       *      }
       *  }
       *  EOSIO_DISPATCH( addressbook, (myaction) )
       ...
       * "); itr--; eosio::check(itr->account_name == name("dan"), "Lock arf, Incorrect Second
       * Record");
       *      }
       *  }
       *  EOSIO_DISPATCH( addressbook, (myaction) )
      ...
      /**
       *  Searches for the `object_type` with the lowest primary key that is greater than or equal
       * to a given primary key.
       *  @ingroup multiindex
       *
       *  @param primary - Primary key that establishes the target value for the lower bound search.
       *  @return An iterator pointing to the `object_type` that has the lowest primary key that is
       * greater than or equal to `primary`. If an object could not be found, it will return the
       * `end` iterator. If the table does not exist** it will return `-1`.
       *
       *  Example:
       *
       *  @code
       ...
       *        eosio::check(itr->account_name == name("brendan"), "Lock arf, Incorrect First Lower
       * Bound Record "); itr++; eosio::check(itr->account_name == name("dan"), "Lock arf, Incorrect
       * Second Lower Bound Record"); itr++; eosio::check(itr == zip_index.end(), "Lock arf,
       * Incorrect End of Iterator");
       *      }
       *  }
       *  EOSIO_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      /**
       *  Searches for the `object_type` with the lowest primary key that is greater than a given
       * primary key.
       *  @ingroup multiindex
       *
       *  @param primary - Primary key that establishes the target value for the upper bound search
       *  @return An iterator pointing to the `object_type` that has the highest primary key that is
       * less than or equal to `primary`. If an object could not be found, it will return the `end`
       * iterator. If the table does not exist** it will return `-1`.
       *
       *  Example:
       *
       *  @code
       ...
       *        eosio::check(itr->account_name == name("dan"), "Lock arf, Incorrect First Upper
       * Bound Record "); itr++; eosio::check(itr == zip_index.end(), "Lock arf, Incorrect End of
       * Iterator");
       *      }
       *  }
       *  EOSIO_DISPATCH( addressbook, (myaction) )
       ...
       *
       *  Notes:
       *  Intended to be used in tables in which the primary keys of the table are strictly intended
       * to be auto-incrementing, and thus will never be set to custom values by the contract.
       * Violating this expectation could result in the table appearing to be full due to inability
       * to allocate an available primary key. Ideally this method would only be used to determine
       * the appropriate primary key to use within new objects added to a table in which the primary
       * keys of the table are strictly intended from the beginning to be autoincrementing and thus
       * will not ever be set to custom arbitrary values by the contract. Violating this agreement
       * could result in the table appearing full when in reality there is plenty of space left.
       *
       *  Example:
       *
       *  @code
      ...
      uint64_t available_primary_key() const
      {
         ...
         eosio::check(_next_primary_key < no_available_primary_key,
       ...
       *
       *  @return An index of the appropriate type: Primitive 64-bit unsigned integer key (idx64),
       * Primitive 128-bit unsigned integer key (idx128), 128-bit fixed-size lexicographical key
       * (idx128), 256-bit fixed-size lexicographical key (idx256), Floating point key, Double
       * precision floating point key, Long Double (quadruple) precision floating point key
       *
       *  Example:
       *
       *  @code
       ...
       *    };
       *    public:
       *      addressbook(name receiver, name code, datastream<const char*> ds):contract(receiver,
       * code, ds) {} typedef eosio::multi_index< name("address"), address, indexed_by< name("zip"),
       * const_mem_fun<address, uint64_t, &address::by_zip> > address_index; void myaction() {
       *        // create reference to address_index  - see emplace example below
       *        // add dan account to table           - see emplace example below
      ...
      auto get_index()
      {
         ...
         auto res = hana::find_if(_indices, [](auto&& in) {
            return std::integral_constant<bool, static_cast<uint64_t>(
                                                    std::decay<typename decltype(+hana::at_c<0>(
                                                        in))::type>::type::index_name) ==
      ...
      auto get_index() const
      {
         ...
         auto res = hana::find_if(_indices, [](auto&& in) {
            return std::integral_constant<bool, static_cast<uint64_t>(
                                                    std::decay<typename decltype(+hana::at_c<1>(
                                                        in))::type>::type::index_name) ==
                                                    static_cast<uint64_t>(IndexName)>();
         });
       ...
       *  @param constructor - Lambda function that does an in-place initialization of the object to
       * be created in the table
       *
       *  @pre A multi index table has been instantiated
       *  @post A new object is created in the Multi-Index table, with a unique primary key (as
       * specified in the object).  The object is serialized and written to the table. If the table
      ...
      const_iterator emplace(name payer, Lambda&& constructor)
      {
         ...
         eosio::check(
             _code == current_receiver(),
             "cannot create objects in table of another contract");  // Quick fix for mutating db
                                                                     // using multi_index that
                                                                     // shouldn't allow mutation.
                                                                     // Real fix can come in RC2.
         ...
         auto itm = std::make_unique<item>(this, [&](auto& i) {
            ...
            // using malloc/free here potentially is not exception-safe, although WASM doesn't
            // support exceptions
            ...
            if (pk >= _next_primary_key)
               _next_primary_key =
                   (pk >= no_available_primary_key) ? no_available_primary_key : (pk + 1);

            hana::for_each(_indices, [&](auto& idx) {
               typedef typename decltype(+hana::at_c<0>(idx))::type index_type;

               i.__iters[index_type::number()] =
                   secondary_index_db_functions<typename index_type::secondary_key_type>::
                       db_idx_store(_scope, index_type::name(), payer.value, obj.primary_key(),
                                    index_type::extract_secondary_key(obj));
            });
         });
       ...
       *  @pre payer is a valid account that is authorized to execute the action and be billed for
       * storage usage.
       *
       ...
       *  @post If payer is the same as the existing payer, payer only pays for the usage difference
       * between existing and updated object (and is refunded if this difference is negative).
       *  @post If payer is different from the existing payer, the existing payer is refunded for
       * the storage usage of the existing object.
       *
       *  Exceptions:
      ...
      void modify(const T& obj, name payer, Lambda&& updater)
      {
         ...
         eosio::check(
             _code == current_receiver(),
             "cannot modify objects in table of another contract");  // Quick fix for mutating db
                                                                     // using multi_index that
                                                                     // shouldn't allow mutation.
                                                                     // Real fix can come in RC2.
         ...
         auto& mutableobj = const_cast<T&>(obj);  // Do not forget the auto& otherwise it would make
                                                  // a copy and thus not update at all.
         updater(mutableobj);

         eosio::check(pk == obj.primary_key(),
         ...
         hana::for_each(_indices, [&](auto& idx) {
            ...
            if (memcmp(&hana::at_c<index_type::index_number>(secondary_keys), &secondary,
                       sizeof(secondary)) != 0)
            {
               ...
               if (indexitr < 0)
               {
                  ...
                  indexitr = mutableitem.__iters[index_type::number()] =
                      secondary_index_db_functions<typename index_type::secondary_key_type>::
                          db_idx_find_primary(_code.value, _scope, index_type::name(), pk,
                                              temp_secondary_key);
               }
       ...
       *
       *  @param primary - Primary key value of the object
       *  @return An iterator to the found object which has a primary key equal to `primary` OR the
       * `end` iterator of the referenced table if an object with primary key `primary` is not
       * found.
       *
       *  Example:
       *
      ...
      {
         auto itr2 =
             std::find_if(_items_vector.rbegin(), _items_vector.rend(),
                          [&](const item_ptr& ptr) { return ptr._item->primary_key() == primary; });
         if (itr2 != _items_vector.rend())
            return iterator_to(*(itr2->_item));

         auto itr = internal_use_do_not_use::db_find_i64(_code.value, _scope,
                                                         static_cast<uint64_t>(TableName), primary);
         if (itr < 0)
            return end();
       ...
       *  @return An iterator to the found object which has a primary key equal to `primary` OR
       * throws an exception if an object with primary key `primary` is not found.
       */

      const_iterator require_find(uint64_t primary,
       ...
       *  @post The existing payer for storage usage of the object is refunded for the table and
       * secondary indices usage of the removed object, and if the table and indices are removed,
       * for the associated overhead.
       *
       *  @return For the signature with `const_iterator`, returns a pointer to the object following
       * the removed object.
       *
       *  Exceptions:
       ...
       *        eosio::check(itr != addresses.end(), "Address for account not found");
       *        addresses.erase( itr );
       *        eosio::check(itr != addresses.end(), "Everting lock arf, Address not erased
       * properly");
       *      }
       *  }
       *  EOSIO_ABI( addressbook, (myaction) )
      ...
      void erase(const T& obj)
      {
         ...
         eosio::check(
             _code == current_receiver(),
             "cannot erase objects in table of another contract");  // Quick fix for mutating db
                                                                    // using multi_index that
                                                                    // shouldn't allow mutation.
                                                                    // Real fix can come in RC2.
         ...
         auto itr2 =
             std::find_if(_items_vector.rbegin(), _items_vector.rend(),
                          [&](const item_ptr& ptr) { return ptr._item->primary_key() == pk; });

         eosio::check(itr2 != _items_vector.rend(),
         ...
         hana::for_each(_indices, [&](auto& idx) {
            ...
            if (i < 0)
            {
               ...
               i = secondary_index_db_functions<typename index_type::secondary_key_type>::
                   db_idx_find_primary(_code.value, _scope, index_type::name(),
                                       objitem.primary_key(), secondary);
            }
            if (i >= 0)
               secondary_index_db_functions<typename index_type::secondary_key_type>::db_idx_remove(
                   i);
         });
