...
namespace eosio
{
    ...
    *
    *  This helper method enables you to add multiple accounts to accounts to be notified list with
    * a single call rather than having to call the similar C API multiple times.
    *
    *  @ingroup action
    ...
    *
    *  @code
    *  require_recipient("Account1"_n, "Account2"_n, "Account3"_n); // throws exception if any of
    * them not in set.
    *  @endcode
    */
    ...
    */
   inline void require_auth(name n) { internal_use_do_not_use::require_auth(n.value); }
    ...
    */
   inline name current_receiver() { return name{internal_use_do_not_use::current_receiver()}; }
    ...
    *  @return the number of bytes copied to msg, or number of bytes that can be copied if len==0
    * passed
   ...
   /**
    * Get the length of the current action's data field. This method is useful for dynamically sized
    * actions
    *
    ...
    */
   inline bool has_auth(name n) { return internal_use_do_not_use::has_auth(n.value); }
   ...
   struct action
   {
      ...
      /**
       * Construct a new action object with the given permission, action receiver, action name,
       * action struct
       *
      ...
      /**
       * Construct a new action object with the given list of permissions, action receiver, action
       * name, action struct
       *
       * @tparam T  - Type of action struct, must be serializable by `pack(...)`
   ...
   namespace detail
   {
      ...
      struct get_nth_impl
      {
         static constexpr auto value = get_nth_impl<N, I + 1, Args...>::value;
      };
      ...
      struct get_nth
      {
         static constexpr auto value = get_nth_impl<N, 0, Args...>::value;
      };
      ...
      struct check_types
      {
         static_assert(detail::is_same<
                       typename convert<T>::type,
                       typename convert<
                           typename std::tuple_element<I, deduced<Action>>::type>::type>::value);
    ...
    *
    * @brief Used to wrap an a particular action to simplify the process of other contracts sending
    * inline actions to "wrapped" action. Example:
    * @code
   ...
   {
      constexpr action_wrapper() : code_name(DefaultContract), permissions() {}
      ...
      action_wrapper(eosio::permission_level l) : code_name(DefaultContract), permissions({1, l}) {}
      ...
      constexpr action_wrapper(Code&& code, eosio::name user)
          : code_name(std::forward<Code>(code)), permissions({1, {user, "active"_n}})
      {
      }
   ...
   struct variant_action_wrapper
   {
      ...
      template <typename Code>
      constexpr variant_action_wrapper(Code&& code,
      ...
      action to_action(Args&&... args) const
      {
         ...
         return action(permissions, code_name, action_name,
                       std::tuple_cat(std::make_tuple(var),
                                      detail::deduced<detail::get_nth<Variant, Actions...>::value>{
   ...
   template <typename... Args>
   void dispatch_inline(name code,
                        name act,
 ...
 * @details The send inline action macro is intended to simplify the process of calling inline
 * actions. When calling new actions from existing actions EOSIO supports two communication models,
 * inline and deferred. Inline actions are executed as part of the current transaction. This macro
 * creates an @ref action using the supplied parameters and automatically calls action.send() on
 * this newly created action.
 *
 * Example:
 * @code
 ...
 * This example:
 *       uses the passed in, dereferenced `this` pointer, to call this.get_self() i.e. the
 * eosio.token contract; calls the eosio.token::transfer() action; uses the active permission of the
 * "issuer" account; uses parameters st.issuer, to, quantity and memo. This macro creates an action
 * struct used to 'send()' (call) transfer(account_name from, account_name to, asset quantity,
 * string memo)
 *
 * @param CONTRACT - The contract to call, which contains the action being sent, maps to the @ref
 * account
 * @param NAME - The name of the action to be called, maps to a @ref name
 * @param ... - The authorising permission, maps to an @ref authorization , followed by the
 * parameters of the action, maps to a @ref data.
 */

#define SEND_INLINE_ACTION(CONTRACT, NAME, ...)                                     \
   INLINE_ACTION_SENDER(std::decay_t<decltype(CONTRACT)>, NAME)                     \
   ((CONTRACT).get_self(), BOOST_PP_TUPLE_ENUM(BOOST_PP_VARIADIC_SIZE(__VA_ARGS__), \
