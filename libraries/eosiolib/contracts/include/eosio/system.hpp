...
namespace eosio
{
   ...
   /**
    *  This method will abort execution of wasm without failing the contract. This is used to bypass
    * all cleanup / destructors that would normally be called.
    *
    *  @ingroup system
    ...
    *  @endcode
    */
   inline void eosio_exit(int32_t code) { internal_use_do_not_use::eosio_exit(code); }
    ...
    *
    * @ingroup system
    * @return name of account that sent the current inline action (empty name if not called from
    * inline action)
    */
   inline name get_sender() { return name(internal_use_do_not_use::get_sender()); }
}  // namespace eosio
