...
namespace eosio
{
   ...
   class transaction_header
   {
     public:
       ...
       *
       * @brief Construct a new transaction_header object initialising the transaction header
       * expiration to now + 60 seconds
       */
      ...
      EOSLIB_SERIALIZE(transaction_header,
                       (expiration)(ref_block_num)(ref_block_prefix)(max_net_usage_words)(
                           max_cpu_usage_ms)(delay_sec))
   };

   /**
    *  Class transaction contains the actions, context_free_actions and extensions type for a
    * transaction
    *
    *  @ingroup transaction
    */
   class transaction : public transaction_header
   {
     public:
       ...
       *  @param replace_existing - Defaults to false, if this is `0`/false then if the provided
       * sender_id is already in use by an in-flight transaction from this contract, which will be a
       * failing assert. If `1` then transaction will atomically cancel/replace the inflight
       * transaction
       */
      void send(const uint128_t& sender_id, name payer, bool replace_existing = false) const
      {
         auto serialize = pack(*this);
         internal_use_do_not_use::send_deferred(sender_id, payer.value, serialize.data(),
                                                serialize.size(), replace_existing);
      }
      ...
      EOSLIB_SERIALIZE_DERIVED(transaction,
                               transaction_header,
   ...
   struct onerror
   {
       ...
       *  @ingroup transaction
       */
      static onerror from_current_action() { return unpack_action_data<onerror>(); }

      /**
       * Unpacks and returns a transaction
       */
      transaction unpack_sent_trx() const { return unpack<transaction>(sent_trx); }
    ...
    */
   inline void send_deferred(const uint128_t& sender_id,
                             name payer,
                             const char* serialized_transaction,
                             size_t size,
                             bool replace = false)
   {
      internal_use_do_not_use::send_deferred(sender_id, payer.value, serialized_transaction, size,
                                             replace);
   }
   /**
    ...
    */
   inline size_t transaction_size() { return internal_use_do_not_use::transaction_size(); }
    ...
    */
   inline uint32_t expiration() { return internal_use_do_not_use::expiration(); }
    ...
    *  @param size - amount of context_free_data[index] to retrieve into buff, 0 to report required
    * size
    *  @return size copied, or context_free_data[index].size() if 0 passed for size, or -1 if index
    * not valid
    */
