...
namespace eosio
{
   namespace internal_use_do_not_use
   {
      extern "C"
      {
         [[clang::import_name("check_transaction_authorization")]] int32_t
         check_transaction_authorization(const char*,
                                         uint32_t,
                                         const char*,
                                         uint32_t,
                                         const char*,
                                         uint32_t);
         [[clang::import_name("check_permission_authorization")]] int32_t
         check_permission_authorization(uint64_t,
                                        uint64_t,
                                        const char*,
                                        uint32_t,
                                        const char*,
                                        ...
                                        uint64_t);
         [[clang::import_name(
             "get_permission_last_used")]] int64_t get_permission_last_used(uint64_t, uint64_t);

         [[clang::import_name("get_account_creation_time")]] int64_t get_account_creation_time(
             uint64_t);
      }
   }  // namespace internal_use_do_not_use
    ...
    *  @param pubkeys_size  - size (in bytes) of serialized vector of provided public keys (can be 0
    * if no public keys are to be provided)
    *  @param perms_data - pointer to the start of the serialized vector of provided permissions
    * (empty permission name acts as wildcard)
    ...
    */
   bool check_transaction_authorization(const char* trx_data,
                                        uint32_t trx_size,
                                        ...
                                        uint32_t perms_size)
   {
      return internal_use_do_not_use::check_transaction_authorization(
          trx_data, trx_size, pubkeys_data, pubkeys_size, perms_data, perms_size);
   }

   /**
    *  Checks if a permission is authorized by a provided delay and a provided set of keys and
    * permissions
    *  @ingroup permission
    *
                                       ...
                                       microseconds delay)
   {
      ...
      return internal_use_do_not_use::check_permission_authorization(
          account.value, permission.value, pubkeys_data, pubkeys_size, perms_data, perms_size,
       ...
       const std::set<public_key>& provided_keys = std::set<public_key>())
   {
      ...
      auto res = internal_use_do_not_use::check_transaction_authorization(
          packed_trx.data(), packed_trx.size(), (nkeys > 0) ? packed_keys.data() : (const char*)0,
          (nkeys > 0) ? packed_keys.size() : 0, (nperms > 0) ? packed_perms.data() : (const char*)0,
       ...
       microseconds provided_delay = microseconds{std::numeric_limits<int64_t>::max()})
   {
      ...
      auto res = internal_use_do_not_use::check_permission_authorization(
          account.value, permission.value, (nkeys > 0) ? packed_keys.data() : (const char*)0,
          (nkeys > 0) ? packed_keys.size() : 0, (nperms > 0) ? packed_perms.data() : (const char*)0,
          (nperms > 0) ? packed_perms.size() : 0, static_cast<uint64_t>(provided_delay_us));
   ...
   {
      return time_point(
          microseconds(internal_use_do_not_use::get_account_creation_time(account.value)));
   }
}  // namespace eosio
