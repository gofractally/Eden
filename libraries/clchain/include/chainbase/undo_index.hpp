...
namespace chainbase
{
   ...
   struct offset_node_value_traits
   {
      ...
      {
         return node_ptr{static_cast<Node*>(
             boost::intrusive::get_parent_from_member(&value, &value_holder<value_type>::_item))};
      }
      static const_node_ptr to_node_ptr(const value_type& value)
      {
         return const_node_ptr{static_cast<const Node*>(
             boost::intrusive::get_parent_from_member(&value, &value_holder<value_type>::_item))};
      }
   ...
   {
      return boost::interprocess::allocator<T, S>{a.get_segment_manager()};
   }
   template <typename T, typename S, std::size_t N>
   ...
   class undo_index
   {
     public:
      ...
      undo_index() = default;
      explicit undo_index(const Allocator& a)
          : _undo_stack{a}, _allocator{a}, _old_values_allocator{a}
      {
      }
      ~undo_index()
      {
      ...
      static_assert(std::is_same_v<typename index0_set_type::key_type, id_type>,
      ...
      const value_type& emplace(Constructor&& c)
      {
         ...
         if (!insert_impl<1>(p->_item))
            eosio::check(
                false, "could not insert object, most likely a uniqueness constraint was violated");
         std::get<0>(_indices).push_back(
             p->_item);  // cannot fail and we know that it will definitely insert at the end.
         on_create(p->_item);
         ++_next_id;
         guard1.cancel();
         guard0.cancel();
      ...
      void modify(const value_type& obj, Modifier&& m)
      {
         ...
         bool success = false;
         {
            auto guard0 = scope_exit{[&] {
               if (!post_modify<true, 1>(node_ref))
         ...
         if (!success)
            eosio::check(
                false, "could not modify object, most likely a uniqueness constraint was violated");
      }
      ...
      class removed_nodes_tracker
      {
        public:
         ...
         {
            _removed_values.clear_and_dispose(
                [this](value_type* obj) { _self->dispose_node(*obj); });
         }
         ...
         void remove(const value_type& obj) { _self->remove(obj, *this); }
      ...
      auto track_removed() { return removed_nodes_tracker(*this); }
     ...
     public:
      ...
      void remove_object(int64_t id)
      {
         ...
         if (!val)
            eosio::check(false, "key not found: " + std::to_string(id));
         remove(*val);
      }
      ...
      {
        public:
         session(undo_index& idx, bool enabled) : _index(idx), _apply(enabled)
         {
            if (enabled)
               idx.add_session();
         }
         session(session&& other) : _index(other._index), _apply(other._apply)
         {
            other._apply = false;
         }
      ...
      session start_undo_session(bool enabled) { return session{*this, enabled}; }

      void set_revision(uint64_t revision)
      {
         if (_undo_stack.size() != 0)
            eosio::check(false, "cannot set revision while there is an existing undo stack");

         if (revision > std::numeric_limits<int64_t>::max())
            eosio::check(false, "revision to set is too high");

         if (static_cast<int64_t>(revision) < _revision)
            eosio::check(false, "revision cannot decrease");
      ...
      {
         return std::get<find_tag<Tag, Indices...>::value>(_indices);
      }
      ...
      std::size_t size() const { return std::get<0>(_indices).size(); }

      bool empty() const { return std::get<0>(_indices).empty(); }
      ...
      struct delta
      {
         boost::mpl::iterator_range<typename index0_set_type::const_iterator> new_values;
         boost::mpl::iterator_range<typename list_base<old_node, index0_type>::const_iterator>
             old_values;
         boost::mpl::iterator_range<typename list_base<node, index0_type>::const_iterator>
             removed_values;
      };
      ...
      void undo() noexcept
      {
         ...
         // replace old_values
         _old_values.erase_after_and_dispose(
             _old_values.before_begin(), get_old_values_end(undo_info),
             [this, &undo_info](pointer p) {
         ...
         _removed_values.erase_after_and_dispose(
             _removed_values.before_begin(), get_removed_values_end(undo_info),
             [this, &undo_info](pointer p) {
                if (p->id < undo_info.old_next_id)
                {
                   get_removed_field(*p) =
                       0;  // Will be overwritten by tree algorithms, because we're reusing the color.
                   insert_impl(*p);
                }
                else
                {
      ...
      void squash() noexcept { squash_and_compress(); }
      ...
      void compress_last_undo_session() noexcept { compress_impl(_undo_stack.back()); }
     ...
     private:
      ...
      void compress_impl(undo_state& session) noexcept
      {
         ...
         remove_if_after_and_dispose(
             _removed_values, _removed_values.before_begin(),
             get_removed_values_end(_undo_stack.back()),
             [old_next_id](value_type& v) { return v.id >= old_next_id; },
      ...
      int64_t add_session()
      {
         ...
         _undo_stack.back().removed_values_end =
             _removed_values.empty() ? nullptr : &*_removed_values.begin();
      ...
      {
         dispose_node(static_cast<node&>(*boost::intrusive::get_parent_from_member(
             &node_ref, &value_holder<value_type>::_item)));
      }
      ...
      {
         dispose_old(static_cast<old_node&>(*boost::intrusive::get_parent_from_member(
             &node_ref, &value_holder<value_type>::_item)));
      }
      void dispose(typename list_base<old_node, index0_type>::iterator old_start,
                   typename list_base<node, index0_type>::iterator removed_start) noexcept
      {
         // This will leave one element around.  That's okay, because we'll clean it up the next time.
         if (old_start != _old_values.end())
            _old_values.erase_after_and_dispose(old_start, _old_values.end(),
                                                [this](pointer p) { dispose_old(*p); });
         if (removed_start != _removed_values.end())
            _removed_values.erase_after_and_dispose(removed_start, _removed_values.end(),
                                                    [this](pointer p) { dispose_node(*p); });
      }
      ...
      static node& to_node(value_type& obj)
      {
         return static_cast<node&>(
             *boost::intrusive::get_parent_from_member(&obj, &value_holder<value_type>::_item));
      }
      static node& to_node(const value_type& obj) { return to_node(const_cast<value_type&>(obj)); }
      static old_node& to_old_node(value_type& obj)
      {
         return static_cast<old_node&>(
             *boost::intrusive::get_parent_from_member(&obj, &value_holder<value_type>::_item));
      }
      ...
      {
         return static_cast<decltype(_old_values.cend())>(
             const_cast<undo_index*>(this)->get_old_values_end(info));
      }
      ...
      {
         return static_cast<decltype(_removed_values.cend())>(
             const_cast<undo_index*>(this)->get_removed_values_end(info));
      }
   ...
   template <typename MultiIndexContainer>
   using multi_index_to_undo_index =
       typename multi_index_to_undo_index_impl<MultiIndexContainer>::type;
}  // namespace chainbase
