...
*/
   ...
   void to_key_varuint32(std::uint32_t obj, S& stream)
   {
      ...
      stream.write(static_cast<char>(~(0xFFu >> (num_bytes - 1)) |
                                     (num_bytes == 5 ? 0 : (obj >> ((num_bytes - 1) * 8)))));
   ...
   void to_key_varint32(std::int32_t obj, S& stream)
   {
      ...
      unsigned char high_byte =
          (num_bytes == 5 ? (sign ? 0xFF : 0) : (uobj >> ((num_bytes - 1) * 8)));
   ...
   void to_key(const std::variant<Ts...>& obj, S& stream)
   {
      ...
      std::visit([&](const auto& item) { return to_key(item, stream); }, obj);
   }
   ...
   void to_key(const T& obj, S& stream)
   {
      ...
      {
         clio::reflect<T>::for_each([&](const clio::meta&, auto member) {
