...
namespace clio
{
   ...
   {
      /**
       * This object is created on a type created by the macro, I represents the member number in
       * the containing type. The containing type's first member is a ProxyObject. This function
       * does the pointer math necessary to find the ProxyObject.
       *
       * Alternatively the macro code would have to initialize every member_proxy with this, which
       * would bloat the size of the member_proxy object
       *
       ...
       * Let reinterpet buf as memper_proxy<proxy_impl>*, this makes the address of proxy__ equal
       * to the address of member_proxy<T> because it is the first element.
       *
       * because member_proxy has no values it takes 1 byte in member_proxy and the value of that
       * byte is never read by member_proxy... member_proxy always gets the address of proxy___ and
       * then does offset math.
       *
       */
      constexpr auto proxyptr() const
      {
         return (reinterpret_cast<const ProxyObject*>(reinterpret_cast<const char*>(this) -
                                                      sizeof(*this) * (I + 1)));
      }
      constexpr auto proxyptr()
      {
         return (reinterpret_cast<ProxyObject*>(reinterpret_cast<char*>(this) -
                                                sizeof(*this) * (I + 1)));
      }
      ...
      {
         return (proxyptr()->template get<I, Name, mptr>());
      }
