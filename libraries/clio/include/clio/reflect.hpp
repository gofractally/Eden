...
#define CLIO_REFLECT_ARGS_INTERNAL(r, OP, i, PARAM) BOOST_PP_COMMA_IF(i) BOOST_PP_STRINGIZE(PARAM)

#define CLIO_REFLECT_ARGS_HELPER(METHOD, PARAM_NAMES) \
   BOOST_PP_SEQ_FOR_EACH_I(CLIO_REFLECT_ARGS_INTERNAL, METHOD, PARAM_NAMES)

#define CLIO_REFLECT_FILTER_PARAMS(NAME, IDX, ...)                            \
   {                                                                          \
      CLIO_REFLECT_ARGS_HELPER(METHOD, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__)) \
   }
...
#define CLIO_REFLECT_FOREACH_INTERNAL(r, OP, i, member)     \
   (void)lambda(clio::meta{                                 \
       .name = BOOST_PP_STRINGIZE(member), .number = i + 1, \
       },                                                   \
       &OP::member);
...
#define CLIO_REFLECT_MEMBER_BY_STR_PB_INTERNAL(r, OP, member) \
   ...
   {                                                          \
      (void)lambda(&OP::CLIO_REFLECT_FILTER_NAME member);     \
      return true;                                            \
   }

#define CLIO_REFLECT_MEMBER_BY_IDX_PB_INTERNAL(r, OP, member) \
   case CLIO_REFLECT_FILTER_IDX member:                       \
      (void)lambda(&OP::CLIO_REFLECT_FILTER_NAME member);     \
      return true;
...
#define CLIO_REFLECT_PROXY_MEMBER_BY_IDX_INTERNAL(r, OP, I, member)                         \
   ...
   {                                                                                        \
      return proxy___.call(clio::meta{.number = I + 1, .name = BOOST_PP_STRINGIZE(member)}, \
                                      &OP::member,                                          \
...
#define CLIO_REFLECT_MEMBER_BY_NAME_I_INTERNAL(r, OP, I, member) \
   case clio::hash_name(BOOST_PP_STRINGIZE(member)):             \
      (void)lambda(&OP::member);                                 \
      return true;
...
#define CLIO_REFLECT(QUERY_CLASS, ...)                                                             \
   ...
   {                                                                                               \
      ...
      typedef std::tuple<                                                                          \
          CLIO_REFLECT_MEMBER_TYPE_IDX_HELPER(QUERY_CLASS, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))> \
          struct_tuple_type;                                                                       \
      template <typename L>                                                                        \
      ...
      {                                                                                            \
        ...
        public:                                                                                    \
         ...
         ProxyObject& operator*() { return proxy___; }                                             \
         const ProxyObject& operator*() const { return proxy___; }                                 \
         CLIO_REFLECT_MPROXY_MEMBER_BY_IDX_HELPER(QUERY_CLASS,                                     \
...
#define CLIO_REFLECT_PB(QUERY_CLASS, ...)                                                          \
   ...
   {                                                                                               \
      ...
      {                                                                                            \
         switch (m)                                                                                \
         {                                                                                         \
            CLIO_REFLECT_MEMBER_BY_IDX_PB_HELPER(QUERY_CLASS,                                      \
                                                 BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))            \
         }                                                                                         \
         return false;                                                                             \
      }                                                                                            \
...
#define CLIO_REFLECT_TEMPLATE_OBJECT(QUERY_CLASS, TPARAM, ...)                                     \
   ...
   {                                                                                               \
      ...
      using tuple_type =                                                                           \
          std::tuple<CLIO_REFLECT_MEMBER_TYPE_HELPER(QUERY_CLASS,                                  \
                                                     BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))>       \
   };                                                                                              \
   template <typename T>                                                                           \
