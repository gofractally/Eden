...
namespace clio
{
   ...
   void from_protobuf_object(std::variant<Ts...>& obj, S& stream)
   {
      ...
      {
         std::visit([&](auto& x) { return from_protobuf_member(x, stream); }, obj);
      }
      else
      {
   ...
   void from_protobuf_member(T& obj, S& stream)
   {
      ...
      else if constexpr (reflect<T>::is_struct || is_std_variant<T>::value ||
                         is_std_vector<T>::value || is_std_tuple<T>::value)
      {
         uint32_t size = 0;
         varuint32_from_bin(size, stream);
         if (size > stream.remaining())
   ...
   void from_protobuf_object(T& obj, S& stream)
   {
      while (stream.remaining())
      {
         ...
         if (not reflect<T>::get(number, [&](auto m) {
                ...
                {
                   skip_member(wire_type,
                               stream);  /// we cannot store return value of functions in T
                }
                else
                {
