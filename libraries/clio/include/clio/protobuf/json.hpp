...
namespace clio
{
   namespace protobuf
   {
      ...
      /**
       * Given a json description of the query and an assumed reflected type (used to interpret
       * names to numbers), this function will convert the JSON-query into a protobuf-style query.
       */
      template <typename T>
               ...
               reflect<T>::for_each([&](const meta& ref, auto mptr) {
                  ...
                  {  /// member function ptr
                     if (ref.name == field.name)
                     {
                        ...
                        {
                           result.fields.push_back(
                               field_query{.number = ref.number, .args = to_protobuf(args_value)});
                        }
                     }
                  }
               });
