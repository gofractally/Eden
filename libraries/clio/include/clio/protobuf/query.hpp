...
namespace clio
{
   namespace protobuf
   {
      ...
      {
         std::vector<field_query> fields;
         std::vector<variant_query>
             variant_fields;  /// only apply these fields when the type has them
      };
      ...
      struct query_proxy
      {
         ...
         query_proxy& call(const meta& ref, Mptr mptr, Filter&& filter)
         {
            ...
            else
            {
               ...
               filter(rp);
               q.fields.push_back({.number = ref.number, .filter = std::move(rp->q)});
            }
            return *this;
         }
         ...
         query_proxy& call(const meta& ref, Mptr mptr, Filter&& filter, Args&&... args)
         {
             ...
             *  order to use a filter you must pass all args, if you don't want a filter on the
             * result then you can skip trailing args.
             */
            if constexpr (std::tuple_size<args_tuple>::value == sizeof...(Args))
            {
               ...
               if constexpr (reflect<result_type>::is_struct)
               {
                  typename reflect<result_type>::template proxy<query_proxy> rp;
                  filter(rp);
                  q.fields.push_back({.number = ref.number,
                                      .args = to_protobuf(params),
