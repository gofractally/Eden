...
namespace b1::chain_kv
{
   ...
   struct database
   {
               ...
               std::optional<int> max_open_files = {})
      {
         ...
         auto write_sentinal = [&](const bytes& k) {
            ...
            auto stat =
                rdb->Get(rocksdb::ReadOptions(), rdb->DefaultColumnFamily(), to_slice(k), &v);
            if (stat.IsNotFound())
            {
   ...
   inline int compare_value(const std::shared_ptr<const bytes>& a,
   ...
   struct undo_state
   {
      ...
      std::vector<uint64_t> undo_stack =
          {};  // Number of undo segments needed to go back each revision
      uint64_t next_undo_segment = 0;
   };

   template <typename Stream>
   void pack_undo_segment(Stream& s,
                          const bytes& key,
                          const bytes* old_value,
   ...
   class undo_stack
   {
     ...
     public:
      undo_stack(database& db,
                 const bytes& undo_prefix,
          ...
          : db{db}, undo_prefix{undo_prefix}, target_segment_size{target_segment_size}
      {
         ...
         rocksdb::PinnableSlice v;
         auto stat = db.rdb->Get(rocksdb::ReadOptions(), db.rdb->DefaultColumnFamily(),
                                 to_slice(this->state_prefix), &v);
         if (!stat.IsNotFound())
         ...
         while (rocks_it->Valid())
         {
            ...
            while (ds.remaining())
            {
               ...
               if (old_value)
                  ...
                  check(batch.Delete({key, key_size}),
      ...
      void commit(int64_t revision)
      {
         ...
         if (first_revision < revision)
         {
            ...
            state.undo_stack.erase(state.undo_stack.begin(),
                                   state.undo_stack.begin() + (revision - first_revision));
            uint64_t keep_undo_segment = state.next_undo_segment;
            for (auto n : state.undo_stack)  //
               keep_undo_segment -= n;
            check(batch.DeleteRange(to_slice(create_segment_key(0)),
                                    to_slice(create_segment_key(keep_undo_segment))),
      ...
      void write_changes(cache_map& cache, cache_map::iterator change_list)
      {
         ...
         auto write_segment = [&] {
            ...
            check(batch.Put(to_slice(key), to_slice(segment)),
         ...
         while (it != cache.end())
         {
            if (compare_value(it->second.orig_value, it->second.current_value))
            {
               if (it->second.current_value)
                  ...
                  check(batch.Delete(to_slice(it->first)),
                        "undo_stack::write_changes: rocksdb::WriteBatch::Erase: ");
               if (!state.undo_stack.empty())
               {
                  append_segment([&](auto& stream) {
                     pack_undo_segment(stream, it->first, it->second.orig_value.get(),
                                       it->second.current_value.get());
                  });
               }
            }
   ...
   struct write_session
   {
      ...
      write_session(database& db, const rocksdb::Snapshot* snapshot = nullptr)
      ...
      void set(bytes&& k, const rocksdb::Slice& v)
      {
         ...
         auto stat =
             db.rdb->Get(read_options(), db.rdb->DefaultColumnFamily(), to_slice(k), &orig_v);
         if (stat.IsNotFound())
         {
            auto [it, b] = cache.insert(
                cache_map::value_type{std::move(k), cached_value{0, nullptr, to_shared_bytes(v)}});
            changed(it);
            return;
         }
         ...
         {
            auto [it, b] = cache.insert(cache_map::value_type{
                std::move(k), cached_value{0, to_shared_bytes(orig_v), to_shared_bytes(v)}});
            changed(it);
         }
         else
         {
      ...
      // Mark key as erased in the cache and add to change_list if changed. Bumps `num_erases` to
      ...
      void erase(bytes&& k)
      {
         ...
         auto [it, b] = cache.insert(cache_map::value_type{
             std::move(k), cached_value{1, to_shared_bytes(orig_v), nullptr}});
         changed(it);
      }

      // Fill cache with a key-value pair read from the database. Does not undo any changes (e.g.
      // set() or erase()) already made to the cache. Returns an iterator to the freshly-created or
      ...
      cache_map::iterator fill_cache(const rocksdb::Slice& k, const rocksdb::Slice& v)
      {
         ...
         auto value = to_shared_bytes(v);
         std::tie(it, b) =
             cache.insert(cache_map::value_type{to_bytes(k), cached_value{0, value, value}});
         return it;
      }
     ...
     private:
      struct iterator_impl
      {
         ...
         //
         {
            ...
            check(rocks_it->status(),
                  "view::iterator_impl::iterator_impl: rocksdb::Iterator::Seek: ");
            view.write_session.fill_cache(rocks_it->key(), rocks_it->value());
            rocks_it->Prev();
            check(rocks_it->status(),
                  "view::iterator_impl::iterator_impl: rocksdb::Iterator::Prev: ");
            view.write_session.fill_cache(rocks_it->key(), rocks_it->value());
         ...
         {
            auto x = compare_blob(rocksdb::Slice{key, size},
                                  rocksdb::Slice{prefix.data() + hidden_prefix_size,
         ...
         void lower_bound_full_key(const bytes& full_key)
         {
            ...
            check(rocks_it->status(),
                  "view::iterator_impl::lower_bound_full_key: rocksdb::Iterator::Seek: ");
            ...
            while (!cache_it->second.current_value)
            {
               while (compare_blob(rocks_it->key(), cache_it->first) <= 0)
               {
                  rocks_it->Next();
                  check(rocks_it->status(),
                        "view::iterator_impl::lower_bound_full_key: rocksdb::Iterator::Next: ");
                  view.write_session.fill_cache(rocks_it->key(), rocks_it->value());
               }
               ++cache_it;
            }
         ...
         bool is_valid()
         {
            return cache_it != view.write_session.cache.end() &&
                   cache_it_num_erases == cache_it->second.num_erases;
         }
         ...
         iterator_impl& operator++()
         {
            ...
            do
            {
               while (compare_blob(rocks_it->key(), cache_it->first) <= 0)
               {
                  rocks_it->Next();
                  check(rocks_it->status(),
                        "view::iterator_impl::operator++: rocksdb::Iterator::Next: ");
                  view.write_session.fill_cache(rocks_it->key(), rocks_it->value());
               }
               ++cache_it;
            } while (!cache_it->second.current_value);
         ...
         iterator_impl& operator--()
         {
            if (cache_it == view.write_session.cache.end())
            {
               rocks_it->Seek(to_slice(next_prefix));
               check(rocks_it->status(),
                     "view::iterator_impl::operator--: rocksdb::Iterator::Seek: ");
               cache_it = view.write_session.fill_cache(rocks_it->key(), rocks_it->value());
            ...
            do
            {
               while (compare_blob(rocks_it->key(), cache_it->first) >= 0)
               {
                  rocks_it->Prev();
                  check(rocks_it->status(),
                        "view::iterator_impl::operator--: rocksdb::Iterator::Prev: ");
                  view.write_session.fill_cache(rocks_it->key(), rocks_it->value());
               }
               --cache_it;
            } while (!cache_it->second.current_value);
     ...
     public:
      ...
      class iterator
      {
        ...
        public:
         ...
         // Get key_value at current position. Returns nullopt if at end. Throws if at an erased kv
         // pair. The returned key does not include the view's prefix or the contract.
      ...
      {
         write_session.erase(create_full_key(prefix, contract, k));
      }
   };  // view
