...
inline kv_values get_values(chain_kv::write_session& session,
                            const std::vector<chain_kv::bytes>& keys)
{
   kv_values result;
   for (auto& key : keys)
   {
...
inline kv_values get_matching(chain_kv::view& view,
                              uint64_t contract,
                              const chain_kv::bytes& prefix = {})
{
   kv_values result;
...
inline kv_values get_matching2(chain_kv::view& view,
                               uint64_t contract,
                               const chain_kv::bytes& prefix = {})
{
   kv_values result;
